...
- `crates/hekate-model/src/source.rs` adds an additive schema version 1
...
- `crates/hekate-sim/src/rng.rs` adds `STREAM_COMPLIANCE`. `profile.rs` samples
...
- `crates/hekate-sim/src/compliance.rs` is the documented decision model card:
...
- `sim.rs` computes and records the decision before integrating each profile
  ...
  expose the small record, `hekate-present` projects it onto `SceneBody` and
...
- `crates/hekate-sim/src/compliance.rs` unit tests cover the green, red, and
  yellow boundaries, the inclusive tie-breaker, the zero-compliance limit, and
  the urgency formula.
- `crates/hekate-sim/tests/signal_compliance.rs` covers green/yellow/red at the
...
- `crates/hekate-sim/src/rng.rs` proves an added `compliance` draw leaves the
  `demand` and `profile` sequences byte-identical.
- `apps/hekate-cli/tests/scenarios.rs` runs `red_light_compliance_v1` and
...
- `crates/hekate-model/src/source.rs` adds an optional `stop_line_m` on
...
- `crates/hekate-sim/src/control.rs` is the documented IDM model card:
...
- `crates/hekate-sim/src/signal.rs` is the fixed-time phase state machine:
...
- `crates/hekate-sim/src/sim.rs` tracks path distance under IDM for every
...
- `crates/hekate-sim/src/control.rs` and `signal.rs` unit tests cover the IDM
  bounds, free-flow convergence, stop-line braking, and green/yellow/red phase
  boundaries including cycle wrap.
- `crates/hekate-sim/tests/longitudinal_control.rs` covers the stop-line rest
  ...
  state.
- `apps/hekate-cli/tests/scenarios.rs` runs `car_following_v1` across seeds 0-2
...
- Schema version 1 gains optional `demand` and `profiles` collections in
  `crates/hekate-model/src/source.rs`; `validate.rs` adds `E_DEMAND_UNKNOWN_PORTAL`,
...
- `crates/hekate-sim/src/rng.rs` derives named portable ChaCha streams from the
...
- `crates/hekate-sim/src/demand.rs` generates arrivals (`rate_vph * dt`, with a
...
- `crates/hekate-sim/src/profile.rs` samples a stable `VehicleProfile` (desired
  speed, body size, time gap, acceleration, comfortable braking) from the
  `profile` stream at admission.
- `crates/hekate-sim/src/sim.rs` admits an arrival only when its entry body
...
- `cargo test --workspace --all-features` passes, including the new
  `crates/hekate-sim/tests/vehicle_flow.rs` (arrivals, route assignment, range
  ...
  `apps/hekate-cli/tests/scenarios.rs`. The walking golden trace and Phase 1
...
- `tangle check nodes` — graph check passed on the moved node.
...
2. Stable saturated queues instead of unbounded spawn overlap —
   `crates/hekate-sim/tests/vehicle_flow.rs`
   ...
   separation; `crates/hekate-sim/tests/longitudinal_control.rs`
...
3. Reproducible distributions drawing only from named streams —
   ...
   `crates/hekate-sim/tests/signal_compliance.rs` cover determinism and stream
   isolation; `apps/hekate-cli/tests/scenarios.rs` asserts an identical
   red-light decision trace for the same seed.
4. Green/yellow/red boundaries and deterministic tie-breaking —
   `crates/hekate-sim/src/compliance.rs` covers the green/red/yellow
   boundaries, the inclusive boundary tie-breaker, the zero-compliance limit,
   and `required_deceleration`; `crates/hekate-sim/src/signal.rs` covers
   start-inclusive/end-exclusive phase boundaries and cycle wrap;
   `crates/hekate-sim/src/sim.rs::leader_ties_resolve_to_the_lowest_agent_id`
   covers the documented leader tie-break against a constructed genuine tie;
   `crates/hekate-sim/src/control.rs` covers the IDM bounds.
