...
Delivered `apps/hekate-viewer` as a Bevy-free library plus a binary. The
...
JSON5 file through the same parse/validate/compile path as `hekate-cli`. The
...
- `cargo test -p hekate-viewer`: 11 tests. Clock tests show that the same total
