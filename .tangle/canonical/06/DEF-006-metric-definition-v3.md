...
(`crates/hekate-sim/src/event.rs (Event::Maneuver)`) and the close-pass
observations already closed (`crates/hekate-sim/src/close_pass.rs
...
`apps/hekate-cli/src/run_metrics.rs (METRIC_DEFINITION_VERSION)`, and it is what
the run summary (`apps/hekate-cli/src/run_dir.rs`), the run metrics artifact and
the batch aggregation (`apps/hekate-cli/src/aggregate.rs`), the paired
comparison (`apps/hekate-cli/src/compare.rs`), the convergence report
(`apps/hekate-cli/src/converge.rs`), and the experiment report
(`apps/hekate-cli/src/experiment.rs`) each write. An artifact already written at
...
records (`crates/hekate-sim/src/event.rs`) whose `kind` is `TacticKind::Overtake`
(`crates/hekate-model/src/source.rs`): one record per documented maneuver
transition, carrying `edge: ManeuverEdge` and `from`/`to: ManeuverState`
(`crates/hekate-sim/src/stage.rs`). All four are countable records in the bucket.
...
- **mode pair** — `ModePair::of` (`crates/hekate-sim/src/metrics.rs`) of the
...
- **facility** — the record's `source_facility`, spelled `facility:<name>` from
  `CompiledScenario::facility_name` (`crates/hekate-model/src/compiled.rs`).

## Added: the close-pass families

The close-pass families are read from the closed `ClosePass` observations
(`crates/hekate-sim/src/close_pass.rs (OvertakeObservation)`), one observation
...
(`crates/hekate-sim/src/event.rs`) for a pass on a compiled shared facility, but
...
2. **`close_pass_minimum_clearance_m`** — unit metres (signed). Formula: per
   ...
   `crates/hekate-sim/src/metrics.rs (tick_minimum_clearance_m)`; positive is the
...
3. **one duration series per declared clearance band, keyed by its stable
   ...
   `crates/hekate-model/src/compiled.rs`; `ClearanceBandId`) while the pair was
...
(`apps/hekate-cli/src/run_dir.rs`, `aggregate.rs`, `compare.rs`, `converge.rs`,
...
(`apps/hekate-cli/src/aggregate.rs`, `compare.rs`).
...
- the overtaking families `overtake_attempts`, `overtake_commits`,
  ...
  (`crates/hekate-sim/src/event.rs`, edges `crates/hekate-sim/src/stage.rs`);
- the close-pass families `close_passes`, `close_pass_minimum_clearance_m` (with
  ...
  `ClosePass` observations (`crates/hekate-sim/src/close_pass.rs
  (OvertakeObservation)`, `tick_minimum_clearance_m` in
  `crates/hekate-sim/src/metrics.rs`).

`METRIC_DEFINITION_VERSION`
(`apps/hekate-cli/src/run_metrics.rs`) is fixed at 3; every other consumer already
reads the constant, and the sole hardcoded version literal
(`apps/hekate-cli/tests/experiment_spec.rs`) was updated to the constant. No
