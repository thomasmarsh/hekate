...
- `crates/hekate-sim/src/wrong_way.rs` (`OpposingTraversalTracker`,
  ...
  reason.
- `crates/hekate-sim/src/agent.rs` (`RouteState::wrong_way_decision`): the most
...
- `crates/hekate-sim/src/sim.rs` (`Simulation::opposing_traversals`,
  ...
  already serialized by `apps/hekate-cli/src/trace.rs`, so no event, version, or
...
`cargo test -p hekate-cli` green (including `golden_trace`, which guards the
...
clean; `scripts/check-dependency-direction.sh` OK; `tangle check` passes.
...
(`crates/hekate-sim/src/wrong_way.rs`, `mod interval_tests`).

One pre-existing test changed: `a_connector_handoff_emits_one_facility_transition_event_mapping_its_record`
(`crates/hekate-sim/tests/wrong_way.rs`) asserted that no event at all followed a
