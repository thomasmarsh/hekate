...
observability gap: `hekate_present::load_scenario` negotiates the schema version
...
and `hekate-tui` and `hekate-viewer` open all six Increment 1 fixtures. Phase 1
...
pass; `tangle check` passes (121 nodes); no Phase 1 golden trace, event or
...
presentation in `hekate-present`/viewers was not required by the increment and
