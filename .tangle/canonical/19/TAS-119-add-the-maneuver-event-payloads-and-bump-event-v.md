...
`EVENT_VERSION` 2 → 3 (`crates/hekate-sim/src/event.rs`), the union's single
...
- `order_key` widens to `(u32, u8, u8, u32, u32, u32, u32, u32)` — agent, kind
  ...
  movement, entering).
- `apps/hekate-cli/src/trace.rs (EventRecord)`: 24 variant-specific optional
...
`crates/hekate-model/src/source.rs` and `components.rs` gain `label()` on
...
`SignalColor::label`, no schema change); `apps/hekate-cli/src/run_metrics.rs
...
the three `apps/hekate-cli/tests/golden/*.migrated.trace.sha256` pins, and
...
`tangle check --allow-pending-advance TAS-100`. `DEF-004-metric-definition-v1`
