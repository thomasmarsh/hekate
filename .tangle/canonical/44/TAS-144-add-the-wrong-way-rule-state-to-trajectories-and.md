...
The snapshot route-state sample lives in `crates/hekate-sim/src/snapshot.rs`,
which the parent's write set excluded, so choosing that seam — or a read
surface on `crates/hekate-sim/src/sim.rs` that the recorder reads instead — is
...
- `crates/hekate-sim/src/snapshot.rs` (`RouteStateSample::perceived_rule`,
...
- `crates/hekate-sim/src/sim.rs` (`Simulation::wrong_way_rule_state`): the state
...
- `apps/hekate-cli/src/trajectories.rs` (`trajectory_columns`, `TrajectorySample`,
...
manifest's sampling policy alone. Nothing was added to `crates/hekate-sim/src/wrong_way.rs`
...
Evidence, all in this worktree: `cargo test -p hekate-sim` green (all suites;
`--lib` 243 passed, `--test route_relative_state` 10 passed with 3 new tests);
`cargo test -p hekate-cli` green (including `trajectories` 12 passed with 2 new,
...
(`crates/hekate-sim/tests/route_relative_state.rs`,
...
(`apps/hekate-cli/src/trajectories.rs`,
...
(`apps/hekate-cli/tests/trajectories.rs`,
...
(`apps/hekate-cli/tests/replay.rs`,
