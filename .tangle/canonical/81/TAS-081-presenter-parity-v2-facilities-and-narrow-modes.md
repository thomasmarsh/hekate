...
`hekate-present` loads schema-version-2 documents (version 1 through migration,
...
dimensions. `hekate-tui` and `hekate-viewer` open the checked-in
...
and narrow-mode data landed, but `hekate-present::load_scenario` still used the
version-1 reader, so the v2 narrow fixtures could not be opened by either viewer.

# Done when

- `hekate_present::load_scenario` negotiates the schema version through
...
- `hekate-tui` and `hekate-viewer` open all six `scenarios/phase2/inc1/*_v2.json5`
...
`crates/hekate-present/src/scenario.rs` reads the file and calls
`hekate_model::parse_scenario_document`, then dispatches on `ScenarioDocument`:
...
this build cannot read, mirroring `apps/hekate-cli/src/lib.rs`. `hekate-present`
still does not depend on `hekate-cli`; nothing in `hekate-model` or `hekate-sim`
changed.

## Scene additions and format version

`crates/hekate-present/src/scene.rs`:
...
- Terminal character backend (`apps/hekate-tui/src/raster.rs`): `draw_geometry`
...
- Terminal pixel/Kitty backend (`apps/hekate-tui/src/pixel.rs`): the same two
...
- Bevy viewer (`apps/hekate-viewer/src/lib.rs`, `src/main.rs`): `body_visuals`
  ...
  presenter names a mode or scenario (`apps/hekate-tui/tests/presenter_no_special_case.rs`
  still passes).
...
- `crates/hekate-present/tests/v2_fixtures.rs` (new):
...
- `apps/hekate-tui/tests/v2_fixture_sessions.rs` (new):
...
- `cargo test -p hekate-present` — passes (44 lib + 2 `scene_golden` + 2
...
- `cargo run -q -p hekate-tui -- scenarios/phase2/inc1/narrow_signal_v2.json5 20260913 --backend ascii </dev/null`
...
- `./target/debug/hekate-viewer <fixture> 20260913` for all six fixtures — each
...
- `tangle check` — `graph check: passed (123 nodes)` while this node was still
  in `proposed/`.

## Files changed

`crates/hekate-present/src/scenario.rs`, `crates/hekate-present/src/scene.rs`,
`crates/hekate-present/src/lib.rs`,
`crates/hekate-present/tests/v2_fixtures.rs` (new),
`apps/hekate-tui/src/raster.rs`, `apps/hekate-tui/src/pixel.rs`,
`apps/hekate-tui/tests/v2_fixture_sessions.rs` (new),
`apps/hekate-viewer/src/lib.rs`, `apps/hekate-viewer/src/main.rs`,
...
required golden update, and no other node owns it). No `crates/hekate-model/**`,
`crates/hekate-sim/**`, `crates/hekate-cli/**`, `baselines/**`, other `docs/**`,
...
- The viewer's `draw_geometry` is an ECS system with no unit test (like every
  ...
  frame-level viewer test, the shared `hekate-present` tests, and the six
