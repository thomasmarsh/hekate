...
- `validate_v2` remains total on every existing version-2 fixture; `cargo test -p
  hekate-model` passes with no Phase 1 regression.
...
[[TAS-067-mode-template-compilation]]. Owns `crates/hekate-model/src/validate.rs`
...
fixture in `crates/hekate-model/tests/facility_validation.rs` (plus one compiled
...
`crates/hekate-model/src/validate.rs` (rules, codes, shared
`required_profile_params`), `crates/hekate-model/src/source.rs`
(`ModeBodySource::Capsule`, `SpeedLimitMps`), `crates/hekate-model/src/mode_template.rs`
(capsule body compile, speed-policy and required-profile consumers),
`crates/hekate-model/src/compiled.rs` (capsule arms in the v1-view profile
materializers), `crates/hekate-model/src/lib.rs` (`SpeedLimitMps` export),
`schemas/scenario-source.schema.json` (regenerated), and the new
`crates/hekate-model/tests/facility_validation.rs`. No `scenarios/**`,
`baselines/**`, `crates/hekate-sim/**`, `crates/hekate-present/**`, or `apps/**`
file changed; no Phase 1 golden trace or baseline changed.

## Acceptance

- `cargo test -p hekate-model` — passes (74 lib + 20 `facility_validation` + all
...
- `cargo test -p hekate-cli --test baseline --test migration_regression --test scenarios` — pass (14 tests; the Phase 1 baseline and migrated goldens are unchanged).
- `scripts/check-dependency-direction.sh` — `dependency direction OK`.
- `cargo fmt -p hekate-model -- --check` and `cargo clippy -p hekate-model --all-targets` — clean.
