...
summary: hekate-model migrates a version-1 source to canonical normalized version 2 and hekate-cli exposes it as `migrate`.
---
...
`hekate-model`, exposed through a `hekate-cli migrate` subcommand that emits
...
pure transform, and `hekate-cli migrate` exposes that transform.

- **Transform** (`crates/hekate-model/src/migrate.rs`): `MIGRATION_VERSION = 1`
...
- **Canonical form** (`to_canonical_v2_json`): 2-space indentation, the
  ...
  normalized hash covers. Both new items are exported from `hekate-model`.
- **Golden fixtures** (`crates/hekate-model/tests/fixtures/migration/`):
...
- **Model tests** (`crates/hekate-model/tests/migration.rs`): the transform is a
...
- **CLI** (`apps/hekate-cli/src/lib.rs` `migrate_scenario`, `main.rs` `migrate`):
...
- **CLI test** (`apps/hekate-cli/tests/migrate.rs`): runs the built binary on the
...
Evidence: `cargo test -p hekate-model` (76 tests) passes; `cargo test -p
hekate-cli` passes including the new `migrate` test (7 tests); `cargo test
--workspace` passes; `cargo clippy -p hekate-model -p hekate-cli --all-targets`
is clean; `cargo fmt --all -- --check` is clean;
`scripts/check-dependency-direction.sh` reports `dependency direction OK`;
`hekate-cli migrate --help` documents the contract; and `tangle check` passes
