...
and claim-resolution seams in crates/hekate-sim plus focused tests. It supplies
...
- `crates/hekate-sim/src/stage.rs` (uncommitted) — the public
...
- `crates/hekate-sim/src/agent.rs` (uncommitted) — `RouteState` gains `intent`,
  ...
  spawn.
- `crates/hekate-sim/src/sim.rs` (uncommitted) — `request_lateral_maneuver`,
  the four-phase `resolve_maneuvers` pass, and `StepOutput::transitions`.
- `crates/hekate-sim/src/lib.rs` (uncommitted) — re-exports the new seam.
- `crates/hekate-sim/tests/maneuver_lifecycle.rs` (new, 5 tests) — the public
...
- `cargo test -p hekate-sim` — 327 passed, 0 failed across 25 suites, including
...
- `tangle check` while TAS-091 was `proposed` — `graph check: passed (154
  nodes)` (exit 0).
