...
Owns crates/hekate-model/src/compiled.rs, components.rs, mode_template.rs, and
...
`crates/hekate-model` compiles every Increment 2 policy shape fixed by
...
**Resolved shapes** (`crates/hekate-model/src/compiled.rs`):
...
**Mode lateral policy** (`crates/hekate-model/src/mode_template.rs`):
...
(`AgentBehaviorProfile` in `crates/hekate-model/src/components.rs`, attached by
`with_lateral_accel_max_mps2`), so no limit is stored twice.

**Tests**: new `crates/hekate-model/tests/increment2_compiled.rs` (15 tests) over
...
- `cargo test -p hekate-model` — 158 passed, 0 failed (75 lib, then
...
- `cargo check --workspace --all-targets` — clean; the additive compiled shape
  breaks no consumer, so no path outside `hekate-model` needed a closure edit.
...
- `tangle check` — `graph check: passed (154 nodes)` while this node was still
  in `proposed/`. Post-move `tangle check --allow-pending-advance TAS-082` —
  `graph check: passed (154 nodes)`; plain `tangle check` then reports only
...
adds compiled data, accessors, and tests in `hekate-model` only. No new
...
- `lateral_accel_max_mps2` is compiled into the behavior profile when authored,
  ...
  `required_profile_params` (`crates/hekate-model/src/validate.rs`) does not list,
