...
Snapshot, trace, and `hekate-present` scene data carry a body kind and an
...
- `crates/hekate-sim/src/snapshot.rs`: `MotionSample` gains
  `body_kind: hekate_model::BodyKind` and `segments: Vec<BodySegmentSample>`;
...
- `cargo test --workspace` passes (62 suites, 0 failures), including the new
  `hekate-sim` `a_full_snapshot_reports_each_phase1_body_kind_with_no_segments`,
  `hekate-present` `a_body_projects_the_body_kind_and_no_phase1_segments`,
  `segment_poses_interpolate_from_the_previous_frame`, `hekate-cli`
