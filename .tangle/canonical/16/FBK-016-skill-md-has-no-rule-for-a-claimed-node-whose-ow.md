...
Attempted: Execute TAS-042 slice E, whose "# Outcome" asserted "the swept time-of-impact query ... no tick consumer uses a cast, so it is not on the tick path", and whose assignment brief repeated that premise. Validating it against crates/hekate-sim showed that safety::scan_pair calls time_of_impact at safety.rs:281 behind the near-miss Lipschitz certificate, reached every tick from SafetyMonitor::observe.
