...
- `cargo test -p hekate-model` passes with golden compiled output for a valid template.
...
`hekate-model` now compiles validated mode templates into agent component
...
- **A valid template compiles to its component bundle.**
  `crates/hekate-model/src/mode_template.rs` (new) adds the pure compiler
...
- **`cargo test -p hekate-model` passes with golden compiled output.**
  `crates/hekate-model/tests/mode_template_compilation.rs` (new) pins the exact
...
Evidence: `cargo test -p hekate-model` passes (69 unit + 10 new integration
