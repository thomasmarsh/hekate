...
The run-directory half: `apps/hekate-cli/src/run_metrics.rs
...
mode pair, per movement, and per facility, and `apps/hekate-cli/src/lib.rs`
...
Evidence: `apps/hekate-cli/tests/run_metrics.rs
...
`apps/hekate-cli/src/aggregate.rs (close_pass_readings)` carries the run bucket
...
unit and explicit applicability; `apps/hekate-cli/src/compare.rs` pairs the same
cells over the seed bank. No existing matrix cell widened. Evidence:
`apps/hekate-cli/tests/aggregate.rs
(the_close_pass_families_aggregate_over_the_run_and_every_dimension)` and
`apps/hekate-cli/tests/compare.rs
