...
- **Both Phase 1 model cards conform to the template section order.** The IDM
  card in `crates/hekate-sim/src/control.rs` and the waypoint card in
  `crates/hekate-sim/src/pedestrian.rs` keep their existing state, parameters,
  ...
  card index in `crates/hekate-sim/src/controller.rs` now names the template as
  the inventory of record.
- **The model-card drift test passes against the template inventory.**
  `crates/hekate-sim/tests/model_cards.rs` now `include_str!`s the template,
  ...
  interface. `cargo test -p hekate-sim --test model_cards` passes (4 tests).

Evidence: `cargo test -p hekate-sim --test model_cards` passes with the
...
direction OK`; `tangle check` passes while the node is still `proposed`.
