...
layer, never in `hekate-model` or `hekate-sim`.
...
`hekate-model` or `hekate-sim`. Parallel batch execution is an outer loop over
...
The session also migrated the vault to `.tangle/` for Tangle 0.6.0
(commit `edf9463`) and aligned `AGENTS.md` to `tangle check` / `.tangle/`;
...
reports `dependency direction OK`; `tangle check` passes. An independent
...
TTC accessor in `crates/hekate-sim/src/metrics.rs`, and `tests/golden`,
`baselines`, `scenarios`, and `schemas` are untouched.

Per criterion:

1. **CLI commands `validate`, `run`, `batch`, `replay` with documented contracts
   and tests.** `apps/hekate-cli/src/main.rs` declares each subcommand with a
   `long_about` contract (`VALIDATE_LONG_ABOUT`, `RUN_LONG_ABOUT`,
   `BATCH_LONG_ABOUT`, `REPLAY_LONG_ABOUT`); tests
   `apps/hekate-cli/tests/{validate,batch,replay,run_directory}.rs` drive the
   built binary.
2. **Immutable run directory; batch stop/resume unchanged.**
   `apps/hekate-cli/src/run_dir.rs` writes `manifest.json` last and refuses a
   completed or foreign directory without writing;
   `apps/hekate-cli/src/batch.rs` derives resume from disk and clears a partial
...
5. **Aggregation distributions and CIs, disaggregated by mode and movement.**
   `apps/hekate-cli/src/aggregate.rs` (two-sided Student-t interval,
...
7. **Fast/Standard/Fine convergence runner and machine-readable sensitivity
   report.** `apps/hekate-cli/src/converge.rs` (`PRESETS` order, tolerance
...
- **The residual backlog is unaffected.** `TAS-029-bound-red-queue-emergency-cap`
  ...
  [[TAS-044-tangle-feedback-triage]] owns the backlog's clear next, and none
  of them gates this increment.
- **Tooling layout.** The vault now lives at `.tangle/` (Tangle 0.6.0);
