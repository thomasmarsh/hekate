...
- A geometrically different benchmark scenario exists, is validated, and is
  shown to need no change to `hekate-model` or `hekate-sim`.
...
(`tangle hash TAS-047`, bare-ID form, FBK-002) with `--lease-seconds 14400`,
and moved to `.tangle/active/` with `git mv` plus `git add` of the
destination (FBK-013). Write set held: `scenarios/**`,
`experiments/increment6_signal_timing_v1/**`, `apps/hekate-cli/tests/**`, and
...
- `experiments/increment6_signal_timing_v1/seed_bank.json`: the checked-in
  common-random-number seed bank, written by `hekate-cli seed-bank --start 1
  --count 10`, so it is the byte-reproducible Increment 5 artifact
  (`seed_bank_version` 1, seeds 1..10 ascending).
- `apps/hekate-cli/tests/experiment_spec.rs`: the contract suite (5 tests).
- `apps/hekate-cli/tests/scenarios.rs`: `offset_junction_v1` added to both
...
```
git diff --stat 7396058^ 7396058 -- crates/hekate-model crates/hekate-sim
(empty)
```

No line of `crates/hekate-model` or `crates/hekate-sim` changed for the whole
slice, which is what makes the geometry data-only.

## Validation

`apps/hekate-cli/tests/experiment_spec.rs` (5 tests, 1.95 s):
...
- `both_variants_validate_and_run_through_the_cli` — `hekate-cli validate`
  reports each variant valid and `hekate-cli run --seed 1 --ticks 600` produces a
...
printed `dependency direction OK`; `tangle check` printed
...
`tangle check` green), which is what unblocked the move; the `Closes TAS-047`
commit moves the node to `.tangle/resolved/` with its `next` removed. The
...
`rg -n -F 'Depends on [[TAS-047' .tangle` returns nothing, and no other node
...
1. **Both variants validate and run through the existing CLI without source
   ...
   `hekate-cli validate` and `hekate-cli run --seed 1 --ticks 600` on each
...
6. **The five gates pass.** Rerun on the committed tree: 564 tests passed, 0
   ...
   `dependency direction OK`; `tangle check` passed (88 nodes).
