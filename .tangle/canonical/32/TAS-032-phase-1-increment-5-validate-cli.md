...
Add a `validate` command to the Hekate CLI alongside the existing `run`
...
Constraint: no new dependency in `hekate-model` or `hekate-sim`; validate uses
...
- `apps/hekate-cli/src/validate.rs` (new) is the CLI-layer seam.
  ...
  `apps/hekate-cli/src/lib.rs` re-exports both.
- `apps/hekate-cli/src/main.rs` adds `validate <SCENARIO>` next to `run` and
  ...
  stderr.
- `apps/hekate-cli/tests/validate.rs` (new) is the contract suite.

## Evidence

`cargo test -p hekate-cli --test validate` (binary `target/debug/deps/validate-*`),
7 passed:
...
from `tangle hash TAS-032`, claimed by `worker` for 14400 s. Write set:
`apps/hekate-cli/src/**`, `apps/hekate-cli/tests/**`, and this node. No path
outside the set was touched and `apps/hekate-cli/Cargo.toml` is unchanged, so no
...
1. **`validate` is reachable with a documented contract.**
   `apps/hekate-cli/src/main.rs:57` registers the subcommand, `:63`
...
3. **No run artifact is written and no tick executes.**
   ...
   half: `apps/hekate-cli/src/validate.rs:29` (`validate_scenario`) calls only
