...
predictor. Files: `crates/hekate-sim/src/agent.rs`,
`crates/hekate-sim/src/sim.rs`, `crates/hekate-sim/tests/lane_transitions.rs`. No
event or trajectory version changed, and no golden, baseline, or schema was
regenerated.

## Seams

- **Source band** (`crates/hekate-sim/src/agent.rs (RouteState.return_facility)`):
...
- **Return target** (`crates/hekate-sim/src/sim.rs (Simulation::return_offset_m)`):
...
- **Return crossing** (`crates/hekate-sim/src/sim.rs (Simulation::handoff_lateral)`):
  ...
  is owed.
- **Return obstruction**
  (`crates/hekate-sim/src/sim.rs (Simulation::return_leg_obstructed)`): a crossed
...
- **Crossing envelope**
  (`crates/hekate-sim/src/sim.rs (Simulation::steering_envelope)`): widened on the
...
- **Settle edge and request**
  (`crates/hekate-sim/src/sim.rs (Simulation::resolve_maneuvers)` settle phase,
  `crates/hekate-sim/src/sim.rs (Simulation::lateral_request)`): both read
  `Simulation::return_offset_m`, so the leg steers to the crossing offset and
  completes only once that offset has held.

## Evidence

Focused tests in `crates/hekate-sim/tests/lane_transitions.rs`:
...
`cargo test -p hekate-sim --test lane_transitions` (14 passed, 0 failed),
`cargo test -p hekate-sim` (all suites, 0 failed),
