...
Owns the determinism and no-car-defaults tests in `crates/hekate-sim/tests/` and
any replay/batch wiring in `apps/hekate-cli` they require.
...
## Determinism and the seed-bank batch (`apps/hekate-cli/tests/narrow_determinism.rs`)
...
## No-car-defaults check and falsification probe (`crates/hekate-sim/tests/narrow_no_car_defaults.rs`)
...
New: `apps/hekate-cli/tests/narrow_determinism.rs`,
`crates/hekate-sim/tests/narrow_no_car_defaults.rs`,
`scenarios/phase2/inc1/narrow_isolated_seed_bank.json`, and this node's move to
`resolved/`. No production module in `hekate-sim`, `hekate-model`,
`hekate-present`, or `apps/hekate-cli/src` changed; no `baselines/**`,
`schemas/**`, `docs/**`, golden trace, Phase 1 fixture, Phase 1 baseline, or
other node file changed. No additive `hekate-model` accessor was needed.

## Acceptance

- `cargo test -p hekate-sim` and `cargo test -p hekate-cli` — pass, including the
...
- `cargo test --workspace` — passes; Phase 1 golden traces and baselines
  ...
  `apps/hekate-cli/tests/golden/**` was touched).
- Batch over the declared seed bank, twice, reproduces per-seed hashes and event
  streams:
  `cargo run -q -p hekate-cli -- batch scenarios/phase2/inc1/narrow_isolated_straight_v2.json5 --seed-bank scenarios/phase2/inc1/narrow_isolated_seed_bank.json --out-root /tmp/narrow-bank-a --ticks 600`
...
- `cargo fmt --all -- --check` clean;
  `RUSTFLAGS="-D warnings" cargo clippy -p hekate-sim -p hekate-cli --all-targets --all-features`
  clean.
- `scripts/check-dependency-direction.sh` — `dependency direction OK`.
- `tangle check` — `graph check: passed (121 nodes)` while this node was
  still `proposed`.
...
`*.json5` does not treat it as a scenario. Post-move, `tangle check`
