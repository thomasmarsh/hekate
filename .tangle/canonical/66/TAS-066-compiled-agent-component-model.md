...
summary: The hekate-model component model composes each agent's core with body, motion, tactics, access, occupancy, and social state, and derives a small body/motion family for dispatch.
---
...
`hekate-model` compiled data carries each simulated road user as a compact core
...
- `cargo test -p hekate-model` passes with the existing Phase 1 compilation unchanged.
...
`hekate-model` now carries the compiled agent component model, additive to the
compiled scenario. Three `Done when` criteria hold.

- **Every agent-composition component is defined.**
  `crates/hekate-model/src/components.rs` (new) defines the bundle
...
Evidence: `cargo test -p hekate-model` passes (69 unit + 4 new integration tests
in `crates/hekate-model/tests/agent_components.rs`; the integration tests compose
