...
golden/baseline regeneration, and no dependency change in `hekate-model` or
`hekate-sim`. The canonical trace, trace golden, trace hash golden, and Phase 1
...
from `tangle hash TAS-042`; this node moved `.tangle/proposed/` →
`.tangle/active/` as part of the claimed edit. Write set actually used:
`scripts/**` (four new scripts), `perf/**` (new), `apps/hekate-cli/tests/**`
...
- `apps/hekate-cli/tests/release_benchmark.rs` — the end-to-end harness. An
...
contact confirmation in `safety::scan_pair` (`crates/hekate-sim/src/safety.rs:281`,
...
and tests (`crates/hekate-sim/tests/swept_queries.rs`, `tests/safety_events.rs`).
...
1. **A reproducible release-mode benchmark harness writes a machine-readable
   ...
   `apps/hekate-cli/tests/release_benchmark.rs`, an ignored test, which writes
...
3. **The swept-TOI state is recorded as the baseline.** — Recorded above in
   ...
   once, as the safety monitor's per-tick contact confirmation in
   `safety::scan_pair` (`crates/hekate-sim/src/safety.rs:281`) gated behind the
...
6. **The five gates pass on the final tree.** — Rerun after the parent frontier
   ...
   `tangle check` → `graph check: passed (74 nodes)`.
