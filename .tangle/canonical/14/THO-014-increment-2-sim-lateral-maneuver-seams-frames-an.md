...
`crates/hekate-sim` (TAS-095 and its children, TAS-105, TAS-106). Link it by
plain wikilink; load it on demand rather than restating it. Verified at commit
35d8030.

# Seams

- `crates/hekate-sim/src/sim.rs` (~6,300 lines; prefer `path (Symbol)`):
...
Focused: `cargo test -p hekate-sim` (358 tests at commit 35d8030). Node
