...
Area [[IDX-001-hekate]]. This coordinating task owns the Increment 6 outcome in
...
`rg -n -F 'Depends on [[DEF-004-metric-definition-v1]] at context_rev ' .tangle`
...
[[TAS-044-tangle-feedback-triage]] with the `FBK-001`..`FBK-018` backlog
...
CLI/output layer, never in `hekate-model` or `hekate-sim`; `f64`/`glam::DVec2`;
single-threaded state-affecting tick; stable ordering with explicit tie-breakers
(ascending `AgentId`); no Bevy types in `hekate-model` or `hekate-sim`; parallel
...
1. **Two variants, scenario-data only, spec + seed bank** —
   ...
   are checked in. Asserted by `apps/hekate-cli/tests/experiment_spec.rs`.
2. **Immutable run directories and a concise report** —
   ...
   `apps/hekate-cli/src/run_dir.rs:105-108` with a test.
...
7. **Same manifest reproduces the canonical event stream** — `replay --verify`
   ...
   that broke this is fixed in `apps/hekate-cli/src/batch.rs:438-441` with
...
8. **Geometrically different benchmark without simulator change** —
   ...
   the unmodified kernel; no scenario id is special-cased in `hekate-model` or
   `hekate-sim`.
...
10. **Every direct child resolved or disposed** — TAS-046, TAS-047, TAS-048,
    TAS-049, and TAS-050 are all in `.tangle/resolved/` with populated
    results.
11. **Five gates on the final tree** — `cargo test --workspace --all-features`
    ...
    --check`, `./scripts/check-dependency-direction.sh`, and `tangle check`
...
- **Out of scope and left proposed or blocked:**
  ...
  [[TAS-044-tangle-feedback-triage]] with the `FBK-001`..`FBK-022` backlog,
