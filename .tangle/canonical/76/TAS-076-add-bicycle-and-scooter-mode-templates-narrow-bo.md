...
- A test proves the two templates differ from each other and from
  ...
  (no id branch).
- `cargo test -p hekate-model` passes with the existing golden compiled
  templates unchanged.
...
`crates/hekate-model/src/mode_template.rs`, `crates/hekate-model/src/components.rs`,
...
`crates/hekate-model/tests/fixtures/narrow_mode_templates_v2.json5` (new) is a
...
`crates/hekate-model/tests/narrow_mode_templates.rs` (new) proves the fixture
...
behavior profile, `crates/hekate-model/src/components.rs`
...
`crates/hekate-model/src/mode_template.rs` (`compiled_profile`) selects
...
`crates/hekate-model/tests/narrow_mode_templates.rs`:
...
`crates/hekate-model/src/components.rs` (two `AgentBehaviorProfile` fields, the
`narrow_wheeled` constructor, two getters, one unit test),
`crates/hekate-model/src/mode_template.rs` (`compiled_profile` selects the
narrow constructor by body/motion), the new
`crates/hekate-model/tests/fixtures/narrow_mode_templates_v2.json5`, and the new
`crates/hekate-model/tests/narrow_mode_templates.rs`. No `crates/hekate-sim/**`,
`crates/hekate-present/**`, `apps/**`, `baselines/**`, `schemas/**`, or `docs/**`
file changed; no existing golden compiled template changed.

## Acceptance

- `cargo test -p hekate-model` — passes (75 lib + 8 new `narrow_mode_templates`
...
- `cargo fmt -p hekate-model -- --check` and `cargo clippy -p hekate-model
  --all-targets` — clean.
