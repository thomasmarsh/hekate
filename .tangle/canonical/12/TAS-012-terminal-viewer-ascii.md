...
A `hekate-tui` binary renders the shared `SceneFrame` as colored character
...
All Done-when criteria are met. `hekate-tui` is a crossterm terminal backend on
the shared presentation contract.

- New `apps/hekate-tui` depends only on `hekate-model`, `hekate-sim`,
  `hekate-present`, `crossterm`, and `glam`. `crates/hekate-present` and the
...
- Pause, single step, 1x/4x/max speed, restart same and next seed, pan, zoom,
  ...
  decision.
- Tests: `cargo test -p hekate-tui` runs 27 green tests, including a
