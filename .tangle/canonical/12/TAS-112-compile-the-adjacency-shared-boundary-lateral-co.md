...
- The derivation reuses one implementation in `crates/hekate-model` rather than
...
- `crates/hekate-model/src/validate.rs`'s `shared_boundary_midpoint` is the one
...
`crates/hekate-model/tests/increment2_compiled.rs`, plus a
`shared_boundary_midpoint` unit test in `validate.rs`.

Evidence: `cargo test -p hekate-model` (all green), `cargo test --workspace`
