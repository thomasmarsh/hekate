...
- `apps/hekate-cli/src/run_metrics.rs (METRIC_DEFINITION_VERSION)` is 3, and
...
`METRIC_DEFINITION_VERSION` (`apps/hekate-cli/src/run_metrics.rs`) is 3. The sole
hardcoded version literal (`apps/hekate-cli/tests/experiment_spec.rs`) now reads
...
Validation: `cargo test -p hekate-cli` (all suites), clippy `-D warnings`, fmt
`--check`, the dependency-direction check, and `tangle check` all pass.
