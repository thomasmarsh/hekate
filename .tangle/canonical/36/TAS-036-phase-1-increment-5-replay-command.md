...
Add a `replay` command to the Hekate CLI alongside `validate`, `run`, and
...
- `apps/hekate-cli/src/replay.rs` (new) owns reproduction.
...
- `apps/hekate-cli/src/main.rs` adds `replay <RUN_DIR> [--verify]` (`:82`,
...
- `apps/hekate-cli/src/lib.rs` exports `replay_run_directory` and `ReplayError`
  (`:33`); `apps/hekate-cli/tests/replay.rs` (new) is the contract suite.
...
`cargo test -p hekate-cli --test replay` (7 passed) and
`cargo test -p hekate-cli --lib` (29 passed, including the 3 new unit tests in
`replay.rs`):
...
from `tangle hash TAS-036`, claimed by `worker` for 14400 s. Write set:
...
None. `flate2`, already a `hekate-cli` dependency from B1, decodes the recorded
stream; `apps/hekate-cli/Cargo.toml` and `Cargo.lock` are unchanged.

## Preservation

No file under `tests/golden/`, `baselines/`, `scenarios/`, `schemas/`, or the
kernel crates changed; `EVENT_VERSION` stays 2
(`crates/hekate-sim/src/event.rs:66`). Replay composes the existing
...
5. **The canonical trace format, trace golden, trace hash golden, Phase 1
   ...
   implementation commit touches only `apps/hekate-cli/src/{lib.rs,main.rs,replay.rs}`
   and `apps/hekate-cli/tests/replay.rs` plus the node move; no file under
   `tests/golden/`, `baselines/`, `scenarios/`, `schemas/`, or the kernel crates
   changed, and `EVENT_VERSION` stays 2
   (`crates/hekate-sim/src/event.rs:66`). The golden, hash-golden, baseline, and
