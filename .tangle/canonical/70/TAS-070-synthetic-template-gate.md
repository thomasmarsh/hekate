...
- **A checked-in synthetic template compiles to components and runs.**
  `crates/hekate-model/tests/fixtures/synthetic_mode_template_v2.json5` (new)
  ...
  `crates/hekate-model/tests/synthetic_mode_template.rs` (new) proves the fixture
  ...
  `crates/hekate-sim/src/sim.rs` adds
...
- **The shared code gained no branch on the synthetic mode.**
  `crates/hekate-sim/tests/synthetic_template_no_branch.rs` (new) includes the
...
Evidence: `cargo test -p hekate-model` passes (69 unit + new integration tests);
`cargo test -p hekate-sim` passes (139 unit + every integration suite, including
