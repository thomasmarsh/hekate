...
`apps/hekate-cli/tests/migration_regression.rs` (new) drives the gate. For every
...
`apps/hekate-cli/tests/golden/<id>.migrated.trace.sha256` (placed beside the suite
...
Evidence: `cargo test` (workspace) passes; `cargo test -p hekate-cli` passes,
including the two `baselines/phase1` checks and the new four-test suite;
`cargo clippy -p hekate-cli --all-targets` is clean; `cargo fmt --all -- --check`
