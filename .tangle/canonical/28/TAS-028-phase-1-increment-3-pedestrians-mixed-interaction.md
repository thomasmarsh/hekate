...
ordering with explicit tie-breakers, no Bevy types in `hekate-model` or
`hekate-sim`, and no schema version 2 (`PHASE_2_PLAN.md` owns it).
...
`crates/hekate-model/src/source.rs` adds four additive schema-version-1
...
`crates/hekate-model/src/validate.rs` adds twelve stable diagnostic codes —
...
`crates/hekate-model/src/compiled.rs` adds dense `WaitingAreaId`,
...
`crates/hekate-sim` makes pedestrians first-class over one shared store:
...
- `crates/hekate-model/src/source.rs` tests parse the new primitives and the
additive defaults.
- `crates/hekate-model/src/validate.rs` tests accept a complete pedestrian
layout and flag every new route, waiting-area, demand, and profile diagnostic;
`crates/hekate-model/src/compiled.rs` tests compile the same layout and check
dense ids, ordered crossings/waiting areas, derived endpoints, and the id map.
- `crates/hekate-model/tests/fuzz_scenarios.rs` generates the new collections so
the "arbitrary sources never panic" property covers them.
- `crates/hekate-sim/tests/pedestrian_flow.rs` (new, 12 tests) covers demand
...
- `crates/hekate-sim/src/rng.rs` proves the added pedestrian stream differs from
...
`tangle check nodes`. The walking trace golden, scene golden, and Phase 1
...
controller in a new `crates/hekate-sim/src/pedestrian.rs` (a public module
...
- `crates/hekate-sim/src/pedestrian.rs` unit tests: closest-arc projection on a
...
- `crates/hekate-sim/tests/pedestrian_control.rs` (new, 11 tests): waypoint
...
`cargo clippy --workspace --all-targets --all-features -- -D warnings`,
`cargo fmt --all --check`, `./scripts/check-dependency-direction.sh`, and
`tangle check nodes`. The walking trace golden, the scene golden, and the
...
`crates/hekate-model/src/source.rs` adds:
...
`crates/hekate-model/src/validate.rs` adds
...
- `crates/hekate-sim/src/signal.rs` adds `PedestrianSignalColor { Walk, DontWalk
...
- `crates/hekate-sim/src/pedestrian_compliance.rs` is the documented decision
...
- `crates/hekate-model/src/{source,validate,compiled}.rs` tests parse the new
  ...
  `crates/hekate-model/tests/fuzz_scenarios.rs` generates the embedded signal
  (including non-positive phases) and the compliance range, so the
  "arbitrary sources never panic" property covers them.
- `crates/hekate-sim/src/pedestrian_compliance.rs` unit tests cover the walk,
  ...
  waiting profile.
- `crates/hekate-sim/tests/pedestrian_compliance.rs` (new, 7 tests): the
...
- `crates/hekate-sim/src/rng.rs` proves a pedestrian `compliance` draw leaves the
...
`crates/hekate-sim/src/index.rs` is the new, documented foundation for
...
- `crates/hekate-sim/src/index.rs` unit tests: both modes are indexed and a
...
- `crates/hekate-sim/tests/vehicle_yielding.rs` (new, 7 tests): a vehicle holds
...
`crates/hekate-sim/src/controller.rs` (new) is the seam. `Simulation` holds
...
`crates/hekate-sim/tests/mixed_interaction.rs` (new, 4 tests) measures that
...
- `crates/hekate-sim/src/controller.rs` tests: the reported model identities,
...
- `crates/hekate-sim/tests/model_cards.rs` (new, 4 tests): reads both cards and
...
- `apps/hekate-cli/tests/scenarios.rs` now covers `mixed_interaction_v1` in both
...
- `Event::Spawned` still does not carry the agent mode, and the presentation
  projection (`crates/hekate-present/src/scene.rs`) still draws every body as a
...
- The five gates pass on the final tree: `cargo test --workspace --all-features`,
  `cargo clippy --workspace --all-targets --all-features -- -D warnings`,
  `cargo fmt --all --check`, `./scripts/check-dependency-direction.sh`, and
  `tangle check nodes`. No schema, golden, or baseline changed.
...
`tangle check nodes` passes (48 nodes). The focused regressions also pass:
...
1. **Cars and pedestrians share the world, index, rule representation, and
   ...
   `SpatialIndex` built over both modes (`crates/hekate-sim/src/index.rs`), the
   authored crossing/movement `RuleKind::Yield` obligation, and `Event::Yielded`
   in the same `Event` enum. Evidence:
   `crates/hekate-sim/tests/mixed_interaction.rs::the_mixed_benchmark_shares_one_world_one_rule_and_one_event_stream`
...
2. **Noncompliance is a choice over a physically possible movement, drawn only
   from the named `compliance` stream.** `crates/hekate-sim/src/pedestrian_compliance.rs`
   ...
   `crates/hekate-sim/tests/pedestrian_compliance.rs::a_noncompliant_pedestrian_crosses_against_a_forbidding_signal`
...
3. **Mixed benchmark completes without NaNs, overlaps, or deadlock, and is
   reproducible.** `crates/hekate-sim/tests/mixed_interaction.rs` sweeps 24
...
4. **Waypoint following, local avoidance, signal-compliance boundaries, and
   yielding have evidence.** `crates/hekate-sim/src/pedestrian.rs` unit tests
   plus `crates/hekate-sim/tests/pedestrian_control.rs`; the
   `pedestrian_compliance.rs` boundary and phase-wrap tests; and
   `crates/hekate-sim/tests/vehicle_yielding.rs`, including the slice G
   backward-movement regression.
5. **Explicit controller interfaces and model cards.**
   `crates/hekate-sim/src/controller.rs` (`VehicleController`,
   `PedestrianController`, `ControllerModels::initial`), the vehicle card at
   `crates/hekate-sim/src/control.rs:3-152`, the pedestrian card at
   `crates/hekate-sim/src/pedestrian.rs:3-190`, and
   `crates/hekate-sim/tests/model_cards.rs` plus the stub-swap replaceability
   test.
