...
- The Parquet dependency lives in the CLI/output layer (`apps/hekate-cli`)
  only; `hekate-model` and `hekate-sim` gain nothing. No scenario-schema,
...
- `apps/hekate-cli/src/trajectories.rs` (new) owns the artifact.
...
including the new `apps/hekate-cli/tests/trajectories.rs` (9 tests) and the 4
...
- The updated `apps/hekate-cli/tests/run_directory.rs` shows the default run
...
from `tangle hash TAS-034`, claimed by `worker` for 14400 s. Write set:
...
Three dependencies added to `apps/hekate-cli/Cargo.toml`, the output layer
...
and the root `Cargo.toml` is untouched. `cargo tree` over `hekate-model` and
`hekate-sim` holds no Parquet, Arrow, flatbuffers, or thrift crate, and
...
5. **The canonical trace format, trace golden, trace hash golden, and Phase 1
   ...
   (`crates/hekate-sim/src/event.rs`); the golden guards
   ...
   golden. `hekate-model` and `hekate-sim` depend on no Parquet, Arrow,
...
6. **The five gates pass on the final tree** (rerun on `faf23f7`,
   ...
   `tangle check nodes` printed `graph check: passed (60 nodes)`).
