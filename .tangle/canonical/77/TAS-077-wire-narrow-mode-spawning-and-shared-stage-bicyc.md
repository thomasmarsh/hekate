...
- `hekate-sim` spawns narrow agents from their compiled mode template and routes
...
- A `bicycle` card and a `scooter` card conform to `docs/model-card-template.md`
  and the drift test `crates/hekate-sim/tests/model_cards.rs` passes.
...
Owns `crates/hekate-sim/src/sim.rs`, `stage.rs`, `controller.rs`, `control.rs`,
...
(the one additive, read-only `hekate-model` accessor: a new private
...
- `crates/hekate-sim/src/narrow.rs` (new): the narrow wheeled family's isolated
...
- `crates/hekate-sim/src/controller.rs` holds the narrow model in
  `ControllerModels` and reports it in `ControllerModelNames { vehicle, narrow,
  pedestrian }`; `crates/hekate-sim/src/agent.rs` derives each agent's
...
`crates/hekate-sim/tests/narrow_mode_no_branch.rs` (new) is the TAS-070-pattern
...
- `crates/hekate-sim/tests/narrow_spawn.rs`
...
despawns.
- `crates/hekate-sim/tests/narrow_mode_no_branch.rs`: the guard above (3 tests).
- `crates/hekate-sim/tests/model_cards.rs`: `every_initial_model_card_states_the_full_model_inventory`
...
`crates/hekate-sim/src/{narrow.rs (new), control.rs (IDM law split out),
...
`crates/hekate-sim/tests/{narrow_spawn.rs (new), narrow_mode_no_branch.rs (new),
model_cards.rs, mixed_interaction.rs}`; `crates/hekate-model/src/compiled.rs`
(the additive `demand_modes` field and `demand_mode` accessor). No
`crates/hekate-present/**`, `apps/**`, `baselines/**`, `schemas/**`, or `docs/**`
...
- `cargo test --workspace` — passes (67 result lines), Phase 1 golden traces
  unchanged (`cargo test -p hekate-cli --test golden_trace --test baseline
...
`crates/hekate-sim/tests/fixtures/narrow_longitudinal_v2.json5` and the test
`crates/hekate-sim/tests/narrow_longitudinal.rs::a_narrow_agent_accelerates_follows_holds_yields_and_completes_a_compiled_facility_route`
