...
- `crates/hekate-sim/src/metrics.rs` — the online interaction metrics (time to
  ...
  (`crates/hekate-sim/src/sim.rs:671`);
- `crates/hekate-sim/src/event.rs` with `crates/hekate-sim/src/safety.rs` — the
  typed event union (`EVENT_VERSION` 2, `event.rs:66`) whose records the
  canonical event stream carries;
- `apps/hekate-cli/src/run_dir.rs` — the immutable run directory the CLI writes
...
1. **Minimum time to collision — `minimum_ttc_s`, unit seconds**
   ...
   because candidate pairs are ascending (`crates/hekate-sim/src/index.rs:361`).
...
The run summary the CLI writes (`apps/hekate-cli/src/run_dir.rs:314`) reports the
...
kernel's run totals (`crates/hekate-sim/src/sim.rs:182`, `:187`). The summary does
...
- **Mode** — `AgentMode` (`vehicle` or `pedestrian`,
  `crates/hekate-sim/src/agent.rs:21`; labels at `agent.rs:30`), resolved per
...
- **Movement** — the chosen key is the tagged union of **`MovementId` for
  ...
  `crates/hekate-model/src/compiled.rs:682`) and pedestrian route
...
`crates/hekate-sim/src/metrics.rs` (minimum TTC, minimum surface separation,
minimum post-encroachment time, region occupancy interval) and the event-count
families of `crates/hekate-sim/src/event.rs` (collisions, near misses,
