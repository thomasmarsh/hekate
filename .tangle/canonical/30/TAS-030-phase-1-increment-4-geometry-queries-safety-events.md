...
`hekate-model` or `hekate-sim`. Do not break existing goldens or baselines
...
`crates/hekate-sim/src/query.rs` defines `BodyShape` (a vehicle is an oriented
...
`hekate_sim::{BroadPhase, Aabb, BodyShape, body_clearance_m, bodies_intersect,
...
- `crates/hekate-sim/tests/geometry_queries.rs`: randomized all-pairs property
...
`hekate_sim::{SweptBody, SweptBroadPhase, TimeOfImpact, time_of_impact,
TOI_TIME_TOLERANCE, body_contact_normal}`. `BodyShape::translated` stays
crate-internal. The tick integration, `SpatialIndex`, and
`crates/hekate-sim/Cargo.toml` are untouched, so no dependency was added.
...
- `crates/hekate-sim/tests/swept_queries.rs`: hand-computed fixtures for a fast
...
`hekate_sim::{Event, EventKind, ControlTransitionKind, RegionKey, ViolationKind,
...
`crates/hekate-sim/tests/*` (new `safety_events.rs`), `apps/hekate-cli`
(`trace.rs` record shapes and header, `tests/scenarios.rs`), `apps/hekate-tui` and
`apps/hekate-viewer` (explicit arms for the new kinds).
...
- Unit tests in `safety.rs`: the swept circle contains every pose of the tick; a
  ...
  edge.
- `crates/hekate-sim/tests/safety_events.rs`: the documented tie-breakers on
...
`crates/hekate-sim/src/metrics.rs` owns the pass and its model card states the
...
`hekate_sim::{InteractionMetrics, MetricMinimum, ModePair, PostEncroachment,
...
- `crates/hekate-sim/tests/metrics.rs` — six differential tests against
...
`crates/hekate-present/src/safety.rs` folds the typed safety records into the
...
`hekate_present::{FrameEvent, EventParticipants, BodyEmphasis, OccupiedRegion,
...
`BodyKind` is removed. No dependency changed: `hekate-present` already depended
on `hekate-model` and `hekate-sim`, no Bevy type entered the kernel or the
...
- `crates/hekate-present/src/scene.rs`: `SceneBody::project` projects
...
- `crates/hekate-present/src/safety.rs`: the participants of every record kind
...
- `crates/hekate-present/src/controller.rs`: observed records reach the projected
  ...
  records.
- `crates/hekate-present/tests/safety_overlays.rs` over the checked-in
...
consume was already covered in `hekate-present` — `src/safety.rs` unit tests
...
- `cargo clippy --workspace --all-targets --all-features -- -D warnings`,
  `cargo fmt --all --check`, `./scripts/check-dependency-direction.sh`, and
  `tangle check nodes` pass on the final tree.
- Dependency direction is unchanged: no crate gained a dependency, `hekate-model`
  and `hekate-sim` still carry no Bevy type, and `f64`/`glam::DVec2` are
  unchanged.
...
`tangle check nodes` passes (54 nodes). An independent read-only review
...
1. **Property tests compare indexed candidates with an all-pairs reference on
   randomized small worlds.** `crates/hekate-sim/tests/geometry_queries.rs`:
   ...
   Evidence: `crates/hekate-sim/src/query.rs` and `src/index.rs` unit tests.
2. **Swept fixtures detect crossing bodies that do not overlap at either tick
   endpoint.** `crates/hekate-sim/tests/swept_queries.rs` covers a fast box
   ...
   `crates/hekate-sim/src/swept.rs`; `src/index.rs` has a unit test that the
...
3. **Event pairs have deterministic ordering and are emitted once according to a
   ...
   `crates/hekate-sim/src/event.rs:32`; `EventKind` (`event.rs:81`) and
   ...
   Tests: `crates/hekate-sim/tests/safety_events.rs` tie-breaker, per-tick order,
...
4. **Fine-step differential tests agree with analytic/simple fixtures within
   declared tolerances.** `crates/hekate-sim/tests/metrics.rs` checks the
   ...
   `crates/hekate-sim/src/metrics.rs`.
5. **`EVENT_VERSION` describes the full event union (F4), and the agent mode is
   ...
   trace (`apps/hekate-cli/src/trace.rs:68,240`) and `SceneBody::project`
   (`crates/hekate-present/src/scene.rs:679-681`) with mode tests at
...
6. **Viewer overlays and inspector links connect an event to its participants.**
   `crates/hekate-present/src/safety.rs` exposes `EventParticipants::of`,
   `others`, `events_involving`, `safety_markers`, `body_emphasis`,
   `occupied_regions`, and `event_summary`; the Bevy viewer
   (`apps/hekate-viewer/src/main.rs`) draws the occupancy ring, emphasis and link
   rings, and markers and lists a body's records; the terminal backends draw the
   same overlays (`b`). Evidence: `crates/hekate-present/tests/safety_overlays.rs`
...
7. **Online TTC and minimum-separation tracking, and conflict-region occupancy
   intervals for PET, have evidence.** `crates/hekate-sim/src/metrics.rs` owns
