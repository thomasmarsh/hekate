...
Attempted: dispatched TAS-075 with a write set of `crates/hekate-model/**` and `scenarios/**`, because the node's stated write set was `validate.rs` and profile parameters.
...
Friction: every leaf handoff left `.tangle/proposed/TAS-019-…` naming a
just-resolved child, so `tangle check` failed until the coordinator advanced the
...
Attempted: dispatched TAS-081 with a write set of `crates/hekate-present/**`,
`apps/hekate-tui/**`, `apps/hekate-viewer/**`, and one doc.
