...
- `crates/hekate-sim/src/sim.rs (Simulation::resolve_maneuvers)`, in the apply
...
- `crates/hekate-sim/src/sim.rs (Simulation::record_facility_transition)`, called
...
- `cargo test -p hekate-sim`: 29 targets, 0 failed, including the new
...
- No golden changed: `apps/hekate-cli/tests/golden_trace.rs`,
  ...
  wrong-way shape, so the new kinds emit nothing there. `apps/hekate-cli/src/trace.rs`
...
fact carried on `crates/hekate-sim/src/agent.rs (RouteState)`, which is outside
...
`tangle check --allow-pending-advance TAS-100`.
