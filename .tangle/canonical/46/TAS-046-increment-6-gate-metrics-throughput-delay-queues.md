...
Consumes the v1 surfaces `crates/hekate-sim/src/metrics.rs`,
`crates/hekate-sim/src/event.rs`, `crates/hekate-cli/src/run_dir.rs`, and the
aggregation/comparison artifacts Increment 5 landed. No new output dependency may
enter `hekate-model` or `hekate-sim`; metric computation may live in the layer
...
- `tangle check` passes, and the working node status, body, and any moves are
...
The always-on observation pass in `crates/hekate-sim/src/metrics.rs`
...
are (`apps/hekate-cli/src/run_metrics.rs:876`):
...
- `METRIC_DEFINITION_VERSION` is 2 (`apps/hekate-cli/src/run_metrics.rs:87`),
...
`crates/hekate-sim/src/metrics.rs` unit tests
...
`cargo clippy --workspace --all-targets --all-features -- -D warnings`,
`cargo fmt --all --check`, `./scripts/check-dependency-direction.sh`, and
`tangle check`. No golden or baseline was regenerated: the pass is read-only
...
to resolved while that route still named it fails `tangle check`
(FBK-011/012). The coordinator advanced that route (commit `7abf598`), and this
node now moves to `.tangle/resolved/` with its `next` removed and a
