...
- `crates/hekate-sim/src/agent.rs` — `RouteState` gains `mode_template`, so a
...
- `crates/hekate-sim/src/stage.rs` — the public `ManeuverReason` code set (the
...
- `crates/hekate-sim/src/narrow.rs` — `pass_side(policy, left_clearance_m,
...
- `crates/hekate-sim/src/sim.rs` — the narrow pass tactic
...
- `crates/hekate-sim/src/lib.rs` — re-exports `ManeuverReason` and `PassSide`.
- `crates/hekate-sim/tests/narrow_passing.rs` (new, 7 tests) — drives the whole
...
- *No named bicycle or scooter branch enters shared controller, query,
  ...
  crates/hekate-sim/src/` returns only pre-existing module-card prose, no code
  branch.

**Acceptance** (exact commands, run from the repository root with the slice in
the working tree):

- `cargo test -p hekate-sim` — 336 passed, 0 failed across 26 suites;
...
- `tangle check` while TAS-093 was `proposed` — `graph check: passed (154
  nodes)` (exit 0).
