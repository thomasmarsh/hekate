...
the smallest required seams in crates/hekate-sim/src/agent.rs, stage.rs,
...
**State representation** (`crates/hekate-sim/src/agent.rs`,
`crates/hekate-sim/src/stage.rs`):
...
**Spawn and pipeline** (`crates/hekate-sim/src/sim.rs`):
...
- `SnapshotDetail::Full` gains `MotionSample::route_state:
  Option<RouteStateSample>` (`crates/hekate-sim/src/snapshot.rs`), the additive
  optional view of the state; the position view is unchanged.
- The trajectory artifact (`apps/hekate-cli/src/trajectories.rs`) appends five
...
- **No `tests/golden/*.trace.jsonl` or `.sha256` golden changed, and none was
  ...
  `MotionSample` literal in `crates/hekate-present/src/scene.rs`'s test module
...
- `crates/hekate-sim/src/agent.rs` unit tests: stable spawn order and columns;
  ...
  label round-trip).
- `crates/hekate-sim/tests/route_relative_state.rs` (new, 7 tests): spawn
...
- `crates/hekate-cli/src/trajectories.rs` tests: declared columns/schema and
...
- `crates/hekate-sim/src/sim.rs` test
...
- `cargo test -p hekate-sim` — 150 lib + 7 `route_relative_state` + all
...
- `cargo test --workspace` — 77 suites pass, 0 failed, including
  `hekate-cli` (`golden_trace` 2, `baseline` 4, `migration_regression` 4,
  `increment6_trace` 2, `trajectories` 10, `run_directory` 11),
  `hekate-present` (`scene_golden` 2, `v2_fixtures` 3), and the sim suites. No
...
- `tangle check` while TAS-088 was `proposed` — `graph check: passed (154
  nodes)`.

## Remaining scope (recorded, not deferred to a new node)

- `RUN_MANIFEST_VERSION` (`apps/hekate-cli/src/run_dir.rs`): the contract's
...
- Attempted: land the whole definition-version closure the contract assigns this
  ...
  other leaves and `apps/hekate-cli/src/run_dir.rs` is not in this leaf's write
