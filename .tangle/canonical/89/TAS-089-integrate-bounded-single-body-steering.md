...
**Bounded steering primitive** (`crates/hekate-sim/src/steering.rs`, new, a
...
- `crates/hekate-sim/src/stage.rs`: `MotionCommand` gains
  ...
  same variant.
- `crates/hekate-sim/src/agent.rs`: `RouteState` carries
...
- `crates/hekate-sim/src/sim.rs`: `route_state_for` now also seeds the envelope
...
- `crates/hekate-sim/src/narrow.rs`: `NarrowProfile` gains
...
**Tests** (`crates/hekate-sim/tests/bounded_steering.rs`, new, 7 tests):
...
`crates/hekate-sim/src/sim.rs` unit test
...
- `cargo test -p hekate-sim` — 152 lib + all suites pass, 0 failed; the new
  `bounded_steering` suite has 7 tests.
- `cargo test --workspace` — every suite passes, 0 failed, including
  `hekate-cli` (`golden_trace`, `baseline`, `migration_regression`,
  `increment6_trace`, `trajectories`, `run_directory`), `hekate-present`
...
- `tangle check` while TAS-089 was `proposed` — `graph check: passed (154
  nodes)`; after the move, `tangle check --allow-pending-advance TAS-087` —
...
- Only a capsule whose sampled `NarrowProfile` carries `lateral_accel_max_mps2`
  ...
  `VehicleProfile`/`sample_profile` (`crates/hekate-sim/src/profile.rs`, outside
...
- Attempted: express the corridor type directly. Friction:
  `hekate_model::UsableLateralInterval` has private fields and no public
  constructor, so a `hekate-sim` integration test cannot build one to drive the
