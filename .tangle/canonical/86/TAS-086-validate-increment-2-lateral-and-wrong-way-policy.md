...
crates/hekate-model/src/validate.rs and focused facility/policy validation tests.
...
case in the new `crates/hekate-model/tests/increment2_validation.rs` whose base
...
site at `crates/hekate-model/src/mode_template.rs` was updated to pass
...
TAS-085's `crates/hekate-model/tests/increment2_compiled.rs` fixture predates
...
`crates/hekate-model/src/validate.rs` (codes, rules, shared
`required_profile_params`), `crates/hekate-model/src/mode_template.rs` (seam
call site), `crates/hekate-model/tests/increment2_compiled.rs` (golden fixture
correction), and the new `crates/hekate-model/tests/increment2_validation.rs`.
No `compiled.rs`/`components.rs` semantics, `docs/schema-v2-contract.md`,
`docs/benchmark-matrix.*`, `hekate-sim/**`, schema, scenario, baseline, or
...
- `cargo test -p hekate-model` — passes (167 tests, including 9 new
...
- `cargo fmt -p hekate-model -- --check` and
  `cargo clippy -p hekate-model --all-targets` — clean.
- `tangle check` — passed (154 nodes) while this node was still `proposed/`;
  after the move, `tangle check --allow-pending-advance TAS-082` is the
