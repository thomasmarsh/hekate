...
**New module** `crates/hekate-sim/src/prediction.rs` (public, re-exported from
...
- *Fixtures*: `crates/hekate-sim/tests/prediction.rs` (12 tests) covers the
...
- `cargo test -p hekate-sim` — 24 suites, 301 passed, 0 failed; the library has
...
- `tangle check` while TAS-090 was `proposed` — `graph check: passed (154
...
- Attempted: read the band edge from a `CompiledFacility` so the input is one
  ...
  `hekate-sim` integration and unit test cannot build one; binding the module to
