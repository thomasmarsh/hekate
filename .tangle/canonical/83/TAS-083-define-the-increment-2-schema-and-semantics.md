...
- Wikilinks — all 37 node links in the document resolve to node files under
  `.tangle/`; `CC-OVERTAKE`/`CC-OPPOSE`, `T-O1`…`T-O3`, `T-H1`/`T-H2`, and
  the four Increment 2 fixture ids resolve in `docs/benchmark-matrix.md`;
  `DEF-004`/`DEF-005` resolve under `.tangle/resolved/`.
...
- Post-move note: `tangle check --allow-pending-advance TAS-082` was run after
...
- Adding a field to `ScenarioSourceV2`, `ModeTemplateSource`, or `AccessSource`
  forces mechanical edits in `crates/hekate-model/src/migrate.rs` (three
  exhaustive literals) and in the exhaustive test literals of
  `crates/hekate-model/tests/mode_template_compilation.rs`,
...
- This contract fixes the tolerance-facing band scalar as a band's
  ...
  (`crates/hekate-model/tests/facility_validation.rs`, `source.rs` tests) author
...
- Attempted: extend the contract while trusting its existing `file:line` seam
  ...
  `tangle check` validates the graph, not a document's citations. Improvement:
