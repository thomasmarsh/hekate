...
`hekate-model` parses a versioned JSON5 scenario containing one guide path and
...
Implemented in `crates/hekate-model`:
...
Evidence: `cargo test -p hekate-model` passes 19 unit and 1 integration tests;
...
reports `dependency direction OK`. `hekate-sim` (TAS-004) consumes
