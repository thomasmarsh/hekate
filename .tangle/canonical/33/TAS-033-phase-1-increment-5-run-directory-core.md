...
Constraint: no new dependency in `hekate-model` or `hekate-sim`. A compression
dependency belongs in the CLI/output layer (`apps/hekate-cli`) and must be
...
`EVENT_VERSION` stays 2, and no new dependency reaches `hekate-model` or
`hekate-sim`.

## What landed

- `apps/hekate-cli/src/run_dir.rs` (new) owns the directory layout.
...
- `apps/hekate-cli/src/trace.rs` adds `canonical_run`, returning the canonical
...
- `apps/hekate-cli/src/main.rs` adds `--run-dir <PATH>` to `run`. The run
...
- `apps/hekate-cli/tests/run_directory.rs` (new) is the contract suite.

## Evidence

`cargo test -p hekate-cli --test run_directory` (10 passed) and
`cargo test -p hekate-cli --lib` (19 passed, including the 5 new unit tests in
`run_dir.rs`):
...
`tangle check nodes` passed.

## Claim and write set

Base hash `71904cc2084cf9c354952d2f4c58d6af8e68352ce30c561f28b667b4cc2f8d93`
from `tangle hash TAS-033`, claimed by `worker` for 14400 s. Write set:
`apps/hekate-cli/src/**`, `apps/hekate-cli/tests/**`,
`apps/hekate-cli/Cargo.toml`, `Cargo.lock` (for the CLI dependency addition
only), and this node's own frontier transition.

## Dependencies reported

One dependency added: `flate2 = "1.1.10"` in `apps/hekate-cli/Cargo.toml`, the
output layer only, with default features (pure-Rust `miniz_oxide`; no C zlib).
`Cargo.lock` gains one line (`hekate-cli` depends on `flate2`, already present
transitively). Neither `hekate-model` nor `hekate-sim` depends on it, and
...
- `build_revision` records the producing crate's version
  (`hekate-cli` `CARGO_PKG_VERSION`), because the workspace stamps no source
...
1. **`run` writes an immutable run directory holding `manifest.json` (JSON),
   ...
   stream.** `apps/hekate-cli/src/run_dir.rs:261` (`write_run_directory`)
   ...
   `apps/hekate-cli/src/main.rs:113` adds `--run-dir` and `:146` writes the
...
2. **The manifest is self-sufficient to reproduce the run and records the
   ...
   stream descriptor; `apps/hekate-cli/src/main.rs:125` loads the source through
   ...
   `hekate_model::MODEL_VERSION`; `run_dir.rs:419`
...
4. **The existing canonical trace format, trace golden, trace hash golden, and
   ...
   stays 2 (`crates/hekate-sim/src/event.rs:66`); the only `trace.rs` change is
   the new `canonical_run` (`apps/hekate-cli/src/trace.rs:131`), which
