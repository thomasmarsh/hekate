...
its ordered segments through the shared `hekate-present` scene seam, with no
...
segments from the shared `hekate-present` scene seam, and no presenter branch
...
- **Box and circle bodies, and ordered segments, render from scene data.**
  `crates/hekate-present/src/scene.rs` gains `BodyShape` and
  ...
  Bevy `apps/hekate-viewer/src/lib.rs` resolves them to shared meshes and
...
- **A viewer test demonstrates multi-kind and multi-segment rendering.**
  `hekate-viewer`'s `a_body_selects_its_mesh_from_kind_and_draws_a_segment_each`
  ...
  their own poses. `hekate-present`'s
...
- **No presenter branch names a scenario or a mode.** The presenters no longer
  ...
  the shared shape list. `apps/hekate-tui/tests/presenter_no_special_case.rs`
  (new) is the TAS-070 source-text guard: it includes `raster.rs`, `pixel.rs`,
  `hekate-viewer/src/lib.rs`, and `hekate-viewer/src/main.rs`, strips each
...
Handoff: with this node resolved, `tangle check` reports the expected
