...
`EVENT_VERSION` stays 2; no dependency change in `hekate-model` or `hekate-sim`
...
`crates/hekate-sim/src/metrics.rs` changed in `crates/`, +91 additive lines;
goldens, baselines, scenarios, and schemas untouched; `.tangle/` tracked).

# Result

Claimed at `content_hash 6eda867b791d2283a5ee50b000dba72d2943d8116f11c5f6ff4df3aa3a85139c`
(the bare digest `tangle hash TAS-043` printed) with
`tangle claim TAS-043 worker --base-hash <hash> --lease-seconds 14400`, and
released with that same base hash before handoff.

Write set as handed: `apps/hekate-cli/src/{batch,compare,lib,main,run_dir}.rs`,
`apps/hekate-cli/tests/{batch,compare,run_directory,run_metrics}.rs`,
...
(`apps/hekate-cli/src/aggregate.rs:655`-`:679`), so mode- and
...
- **F1 (P1) — `comparison.json`'s `unpaired` list is ascending again.**
  ...
  (`apps/hekate-cli/src/compare.rs:1205`-`:1206`), and the doc comments on
  ...
  (`apps/hekate-cli/tests/compare.rs:817`): a two-seed pair whose bucket only
...
- **F2 (P2) — DEF-004's summary requirement holds as written.**
  ...
  (`apps/hekate-cli/src/run_dir.rs:333`), set from
...
- **F4 (P3) — `run` documents its contract.** `RUN_LONG_ABOUT`
  (`apps/hekate-cli/src/main.rs:141`) hangs off the `Run` variant
  (`:113`) with usage, inputs, output, and exit codes, and
  `help_documents_usage_inputs_outputs_and_exit_codes`
  (`apps/hekate-cli/tests/run_directory.rs:445`) pins the help text like
...
- **F5 (P3) — a truncated marker no longer fails the batch closed.** The
  ...
  as a run artifact (`apps/hekate-cli/src/batch.rs:314`, `:334`) so an
  ...
  (`apps/hekate-cli/tests/batch.rs:316`) stages the crash and asserts the re-run
...
6. **Canonical trace, goldens, and Phase 1 baseline unchanged; `EVENT_VERSION`
   ...
   `baselines/`, `scenarios/`, `schemas/`, or `crates/`; `crates/hekate-sim` and
...
7. **The five gates pass on the final tree.** — `cargo test --workspace
   ...
   `tangle check` (75 nodes) recorded in `# Result`.
