...
single constant that fixes it is `apps/hekate-cli/src/run_metrics.rs:87`
(`METRIC_DEFINITION_VERSION`), and it is what the run summary
(`apps/hekate-cli/src/run_dir.rs:440`), the run metrics artifact
(`run_metrics.rs:494`, `:520`), the aggregation
(`apps/hekate-cli/src/aggregate.rs:976`, `:1166`), the comparison
(`apps/hekate-cli/src/compare.rs:761`, `:1404`), and the convergence report
(`apps/hekate-cli/src/converge.rs:766`, `:929`) each write. An artifact already
...
`crates/hekate-sim/src/metrics.rs`: `OperationMetrics`
...
reports (`crates/hekate-sim/src/event.rs:70`), so every despawn is a completed
trip.
...
3. **Stopped delay — `mean_stopped_delay_s` and `total_stopped_delay_s`, unit
   ...
   (`crates/hekate-sim/src/safety.rs:101`). The bucket reports the mean per
...
- **Mode** — `operational.by_mode`, keyed by `AgentMode::label()`
  (`vehicle`, `pedestrian`; `crates/hekate-sim/src/agent.rs:30`), always
...
`crates/hekate-sim/src/metrics.rs` (`OperationMetrics`, `metrics.rs:997`,
observed by the same tick pass at `metrics.rs:1055`) and reported through the run
metrics artifact (`apps/hekate-cli/src/run_metrics.rs:336`, `:438`, `:490`), the
batch aggregation (`apps/hekate-cli/src/aggregate.rs:652`, `:713`), the paired
comparison (`apps/hekate-cli/src/compare.rs:1007`), and the convergence report,
...
(`apps/hekate-cli/tests/run_metrics.rs::metrics_json_reports_the_operational_families`).
