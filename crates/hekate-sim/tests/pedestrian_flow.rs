...
use hekate_model::{CompiledScenario, PathId, PedestrianDemandId, PedestrianRouteId};
use hekate_sim::{
...
fn spawns(sim: &mut Simulation) -> Vec<(AgentId, PathId, f64)> {
    sim.step()
        ...
        .filter_map(|event| match event {
            ...
            | Event::ArticulatedSegmentContact { .. } => None,
        })
        .collect()
}
    ...
    for _ in 0..2000 {
        ...
        for event in &events {
            if let Event::Despawned {
                agent: departed,
                reason: hekate_sim::DespawnReason::ExitedPath,
                ..
            } = *event
                && departed == agent
            {
...
fn same_seed_reproduces_pedestrian_demand_and_profiles() {
    fn run(seed: u64) -> Vec<(u32, Option<hekate_sim::PedestrianProfile>, u64)> {
