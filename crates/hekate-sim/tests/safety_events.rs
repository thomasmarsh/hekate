...
fn the_documented_tie_breakers_order_equal_tick_records() {
    let entries: [Event; 8] = [
        ...
        Event::Spawned {
            ...
            path: hekate_model::PathId::from_index(3),
            distance_m: 0.0,
        },
        Event::Entry {
            agent: AgentId::from_index(0),
        ...
        Event::Entry {
            agent: AgentId::from_index(0),
            region: RegionKey::ConflictRegion(hekate_model::ConflictRegionId::from_index(0)),
        },
        Event::Exit {
            agent: AgentId::from_index(0),
            region: RegionKey::Crossing(CrossingId::from_index(1)),
...
fn pair_records_match_an_independent_swept_query() {
    ...
    for step in &steps {
        for event in &step.events {
            ...
            let near_this_tick = end_clearance_m <= hekate_sim::NEAR_MISS_THRESHOLD_M
                || band_entry(&first_body, &second_body, hekate_sim::NEAR_MISS_THRESHOLD_M)
                    .is_some();
