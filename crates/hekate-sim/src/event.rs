...
//! - the one articulated-chain record, [`Event::ArticulationLimitExceeded`],
//!   from the per-tick hitch check ([`crate::articulated`]). A run with no
//!   `ArticulatedWheeled` agent emits none.
//!
//! # Within-tick order
//!
...
impl Event {
    ...
    pub const fn agent(&self) -> AgentId {
        match self {
            ...
            | Self::ArticulationLimitExceeded { agent, .. } => *agent,
        }
    }
