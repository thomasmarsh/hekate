...
fn a_legacy_version_one_population_carries_no_route_state() {
    ...
    for sample in snapshot.agents() {
        ...
        assert_eq!(motion.body_kind, tangle_model::BodyKind::Box);
        assert!(
...
fn a_box_body_mode_shares_the_route_coordinate_representation() {
    use tangle_model::{ModeBodySource, ProfileRangeSource};
    ...
    template
        .tactics
        .retain(|tactic| *tactic != tangle_model::TacticKind::Pass);
