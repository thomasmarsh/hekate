...
//! [`tangle_sim::predict_maneuver_corridor`].

use std::collections::{BTreeMap, BTreeSet};

use glam::DVec2;
use tangle_model::{CompiledReferencePath, CompiledScenario, parse_scenario_source_v2};
use tangle_sim::{
...
fn the_predictor_settles_the_motor_box_against_front_rear_and_opposing_bodies() {
    ...
    let narrow_leader = PredictedBody {
        ...
        shape: tangle_sim::BodyShape::Box {
    ...
    assert_eq!(
        front.object,
        tangle_sim::LimitingObject::Agent(narrow_leader.id)
    );
    ...
    assert_eq!(
        ...
        PredictionVerdict::Infeasible {
            limiting: tangle_sim::LimitingObject::Agent(rear.id)
        }
    );
    ...
    assert_eq!(
        opposed.verdict,
        PredictionVerdict::Infeasible {
            limiting: tangle_sim::LimitingObject::Agent(opposing.id)
        }
    );
}
...
fn a_target_outside_the_corridor_is_rejected_on_the_band_edge() {
    ...
    assert_eq!(
        prediction.verdict,
        PredictionVerdict::Infeasible {
            limiting: tangle_sim::LimitingObject::BandEdge
        }
    );
}
