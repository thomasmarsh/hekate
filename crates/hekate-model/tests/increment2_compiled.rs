...
fn policy(
    ...
    mode: &str,
    facility: &str,
) -> hekate_model::FacilityTraversalPolicy {
    scenario
        .traversal_policy(
...
fn compiles_the_lateral_policy_of_a_capable_mode_only() {
    ...
    assert!(
        bicycle
            .tactics()
            .supports(hekate_model::TacticalCapability::Pass)
    );
    assert_eq!(
        bicycle
            .profile()
...
fn compiles_the_lateral_acceleration_profile_parameter() {
    ...
    let template: &hekate_model::ModeTemplateSource = &source.mode_templates[0];
    ...
    assert!(
        compiled
            .tactics()
            .supports(hekate_model::TacticalCapability::ChooseLateralPosition)
    );
}
