...
//!
//! `tangle_present::load_scenario` is the only load path either viewer uses, so
//! every `scenarios/phase2/inc1/*_v2.json5` fixture that loads here opens in
//! `tangle-tui` and `tangle-viewer`. Each fixture must load as a native schema
...
use tangle_present::{BodyShape, LoadError, SceneBody, SceneGeometry, load_scenario};
...
fn an_unsupported_schema_version_is_reported_as_such() {
    let path = std::env::temp_dir().join(format!("tangle-present-v3-{}.json5", std::process::id()));
