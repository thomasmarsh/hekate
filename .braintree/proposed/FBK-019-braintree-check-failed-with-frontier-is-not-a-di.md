...
sibling node, `Move this node to .braintree/resolved/ with a Closes TAS-046
...
wikilink — links belong to the route form only — and make `braintree check` name
...
.braintree`, before and after adding `disposition: superseded` to `DEF-004`.
...
(`rg -n '^Depends on \[\[DEF-004' .braintree` matches nothing), which is the
signal that was wanted.

Improvement: prescribe the anchored form `rg -n '^Depends on \[\[DEF-004-metric-definition-v1\]\]
at context_rev ' .braintree` (or without the trailing space), which matches a pin
