...
Friction: 12 of the 22 `file:line` citations had drifted to blank lines or unrelated symbols after Increment 1 landed; nothing checks a document's citations (`braintree check` validates graph structure only).
...
Friction: `braintree check` validates graph integrity, not the `next`/gate structural rules the refresh brief required, so the scout had to hand-roll grep checks.
Improvement: add a `braintree check` lint for the leaf-`next` and gate-structure rules (plain action sentence for a leaf, single direct-child wikilink for a coordinator, `Gated on` only in `# Context`), so a refresh scout does not reimplement them.
