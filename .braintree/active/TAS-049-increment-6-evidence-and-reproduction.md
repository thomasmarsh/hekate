...
next: Resolve this node once the coordinator has advanced TAS-045's next to TAS-050, then commit the resolution.
---
...
(`braintree hash TAS-049`, bare-ID form, FBK-002) with `--lease-seconds 14400`,
and moved to `.braintree/active/` with `git mv` plus `git add` of the destination
(FBK-013). Write set held: `apps/tangle-cli/src/**`, `apps/tangle-cli/tests/**`,
...
- `07543c7` **the golden traces and their test.**
  ...
  pinned by `apps/tangle-cli/tests/increment6_trace.rs`.
...
test `apps/tangle-cli/tests/batch.rs::a_batch_runs_at_its_declared_step_and_replays_at_it`
...
`apps/tangle-cli/tests/converge.rs`'s expectation that the benchmark's
...
- `apps/tangle-cli/tests/increment6_trace.rs` (2 tests) runs the `run` command at
...
It builds the release CLI (or takes `TANGLE_CLI`), runs `experiment` with
...
```sh
tangle-cli experiment experiments/increment6_signal_timing_v1/experiment.json \
...
`crates/tangle-sim` and `crates/tangle-model` are untouched: the whole diff is
`apps/tangle-cli` plus the checked-in artifacts, the goldens, the document, and
...
- `braintree check`: `graph check: passed (89 nodes)`.
