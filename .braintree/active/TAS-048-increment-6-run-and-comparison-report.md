...
(`braintree hash TAS-048`, bare-ID form, FBK-002) with `--lease-seconds 14400`,
and moved to `.braintree/active/` with `git mv` plus `git add` of the
destination (FBK-013). Write set held: `apps/tangle-cli/src/**`,
`apps/tangle-cli/tests/**`, `experiments/increment6_signal_timing_v1/**`, and
...
`apps/tangle-cli/src/experiment.rs` adds `run_experiment` (read the spec, run
...
kernel file changed: the whole diff is `apps/tangle-cli` plus the checked-in
experiment artifacts and the FBK node.

## Gates

All five green on the code tree (`3b34e84`, `575bca4`, `0f6616b`), and `braintree
...
- `cargo test --workspace --all-features`: 576 passed, 0 failed, 1 ignored
  ...
  five in `apps/tangle-cli/tests/experiment.rs`, one each in the aggregate,
