...
tracker. `apps/tangle-cli/src/trace.rs (canonical_run_captured)` calls
...
`ClosePassTracker` (`apps/tangle-cli/src/run_metrics.rs
...
Tests: `crates/tangle-sim/tests/close_pass.rs
...
pair. `apps/tangle-cli/tests/run_metrics.rs
...
Validation: `cargo test -p tangle-sim --test close_pass`, `cargo test -p
tangle-sim --lib`, `cargo test -p tangle-cli --test run_metrics`, `cargo test -p
tangle-cli --test golden_trace --test increment6_trace --test run_directory`,
`cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo fmt
--all --check`, `scripts/check-dependency-direction.sh`, `braintree check`.
