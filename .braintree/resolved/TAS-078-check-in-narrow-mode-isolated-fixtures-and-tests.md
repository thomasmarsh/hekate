...
`crates/tangle-sim/tests/narrow_isolated.rs`, and are listed in the matrix, which
...
`crates/tangle-sim/tests/narrow_isolated.rs` (new), six tests, seed
`20_260_913`:
...
`crates/tangle-sim/tests/narrow_isolated.rs`, and this node's move to
`resolved/`. Modified: `docs/benchmark-matrix.md`,
`docs/benchmark-matrix.json`, and
`apps/tangle-cli/tests/migration_regression.rs` (needed, see below). No
production module in `tangle-sim`, `tangle-model`, `tangle-present`, or
`apps/tangle-cli/src` changed; no `baselines/**`, `schemas/**`, golden trace,
...
- `cargo test -p tangle-sim` — passes (147 lib + all suites, including the six
...
- `cargo fmt --all -- --check` and `RUSTFLAGS="-D warnings" cargo clippy -p
  tangle-sim -p tangle-cli --all-targets --all-features` — clean.
- `scripts/check-dependency-direction.sh` — `dependency direction OK`.
- CLI, each fixture: `cargo run -q -p tangle-cli -- validate
  scenarios/phase2/inc1/<fixture>.json5` reports the document valid, and
  `cargo run -q -p tangle-cli -- run scenarios/phase2/inc1/<fixture>.json5
  --seed 20260913 --ticks 2400` exits 0 with a trace hash for all six.
- `braintree check` — `graph check: passed (120 nodes)` while this node was
  still in `proposed/`.
