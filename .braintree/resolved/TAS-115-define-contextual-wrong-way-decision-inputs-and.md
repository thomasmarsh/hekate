...
Landed `crates/tangle-sim/src/wrong_way.rs` (new, `pub mod wrong_way` plus
re-exports in `crates/tangle-sim/src/lib.rs`):
...
Evidence: `cargo test -p tangle-sim --lib wrong_way` 15 passed (each reason,
...
urgency, and a zero-urgency agent); `cargo test -p tangle-sim --lib` 207 passed;
