...
`scripts/check-dependency-direction.sh` still passes. `braintree check` passed
with the node in `proposed/`. After the move to `resolved/`, `braintree check`
