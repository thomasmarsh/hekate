...
`tangle-sim` now exposes the four `PHASE_2_PLAN.md` controller stages as named
...
- **Each stage is a named interface with an immutable observation and a
  returned command.** `crates/tangle-sim/src/stage.rs` (new) defines
  ...
  the step's diagnostics. `crates/tangle-sim/src/sim.rs` implements all four on
...
- **The controller seam stub-swap test still proves the interface.**
  `crates/tangle-sim/src/controller.rs`'s
...
Focused per-stage tests in `crates/tangle-sim/src/sim.rs` (`#[cfg(test)]`) call
...
Evidence: `cargo test -p tangle-sim` passes (138 unit + integration tests,
...
`braintree check` passes while the node is still `proposed`.
