...
`crates/tangle-sim/src/sim.rs (Simulation::nearest_leader)` handed the leader's
...
## Tests (`crates/tangle-sim/tests/wrong_way.rs`)
...
Evidence, all from this worktree at base `dd455e6`: `cargo test -p tangle-sim
--test wrong_way` 12 passed; `cargo test -p tangle-sim --lib` 215 passed; `cargo
test -p tangle-sim` all 29 targets green (no Phase 1 / Increment 1 / TAS-113..117
regression); `cargo test -p tangle-cli --test golden_trace` and `cargo test -p
tangle-present --test scene_golden` green, so no golden moved; `cargo clippy
...
advance is the coordinator's integration action, verified with `braintree check
