...
- `crates/tangle-sim/src/sim.rs` — the generalized lateral maneuver tactic (the
...
- `crates/tangle-sim/src/profile.rs` — `WheeledLateralLimits` and
...
- `crates/tangle-sim/src/narrow.rs` — `NarrowProfile::lateral_limits`, the
  capsule's projection onto the shared `WheeledLateralLimits`, so both families
  seed the envelope from one shape.
- `crates/tangle-sim/src/agent.rs` — `RouteState::pass_reason` renamed
  `maneuver_reason` for the shared seam.
- `crates/tangle-sim/src/stage.rs` — the `ManeuverReason::Capability` doc now
  names `Pass` or `Overtake`.
- `crates/tangle-sim/tests/motor_overtaking.rs` (new, 6 tests) — the focused
  suite through the public seam.
- `crates/tangle-sim/tests/narrow_passing.rs` — the one accessor rename
  (compile-forced closure).
- `crates/tangle-sim/src/sim.rs` and `src/profile.rs` `#[cfg(test)]` unit tests —
  the box route-state seed and the box lateral-limit sampler.
- `crates/tangle-model/src/mode_template.rs`, `src/components.rs`,
...
`crates/tangle-model/src/mode_template.rs` built `AgentBehaviorProfile::wheeled`
...
- `cargo test -p tangle-sim` — 344 passed, 0 failed across 27 suites
  (`motor_overtaking` 6 passed in ~12 s; lib 187).
- `cargo test --workspace` — 848 passed, 0 failed across 82 suites;
  `cargo test -p tangle-model` 168 passed; no golden regenerated.
- `cargo clippy --workspace --all-targets --all-features -- -D warnings` — clean.
- `scripts/check-dependency-direction.sh` — `dependency direction OK`.
- `braintree check` while TAS-094 was `proposed` — `graph check: passed (154
  nodes)`.
...
- Attempted: keep the write set to `crates/tangle-sim`. Friction: the box
  lateral-mode defect is in `crates/tangle-model`, outside the delegated set;
