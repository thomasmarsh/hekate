...
`crates/tangle-sim/tests/fixtures/narrow_longitudinal_v2.json5` (new): one
...
`crates/tangle-sim/tests/narrow_longitudinal.rs` (new),
...
Added `crates/tangle-sim/tests/fixtures/narrow_longitudinal_v2.json5` and
`crates/tangle-sim/tests/narrow_longitudinal.rs`. No `tangle-sim` production
module, `tangle-model` accessor, `crates/tangle-present/**`, `apps/**`,
...
- `cargo test --workspace` — passes; Phase 1 golden traces unchanged
  (`cargo test -p tangle-cli --test golden_trace --test baseline --test
  migration_regression` passes: 4 + 2 + 4).
- `scripts/check-dependency-direction.sh` — `dependency direction OK`.
- `cargo fmt -p tangle-sim -- --check` and
  `RUSTFLAGS="-D warnings" cargo clippy -p tangle-sim --all-targets --all-features`
  — clean.
- `braintree check` — `graph check: passed (120 nodes)` while this node was still
  in `proposed/`.
