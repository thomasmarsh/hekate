...
their compiled reference-path geometry land in `tangle-model`, additive to
...
## New source shapes (`crates/tangle-model/src/source.rs`)
...
## New compiled surface (`crates/tangle-model/src/compiled.rs`)
...
`crates/tangle-model/src/source.rs`, `crates/tangle-model/src/compiled.rs`,
`crates/tangle-model/src/lib.rs`, `crates/tangle-model/src/migrate.rs`,
`crates/tangle-model/src/mode_template.rs`, `crates/tangle-model/src/validate.rs`,
`crates/tangle-model/tests/mode_template_compilation.rs`, and
...
- `cargo test -p tangle-model` — passes (73 lib + all integration suites);
  includes `schema::tests::checked_in_v2_schema_matches_the_generated_schema`.
- `cargo build --workspace` — passes.
- `cargo test --workspace` — passes.
- `cargo test -p tangle-cli --test baseline --test migration_regression` —
...
- `cargo fmt --all --check` and `cargo clippy -p tangle-model --all-targets` —
  clean.
- `braintree check` — `graph check: passed (118 nodes)` while this node was
  still in `proposed/`.
