...
(`braintree hash TAS-050`, bare-ID form, FBK-002) with `--lease-seconds 14400`,
...
- the recorded replay command above, its stdout format, and the regeneration
  command;
- the Bevy viewer, `cargo run -p tangle-viewer --
  scenarios/experiments/four_leg_pedestrian_ew_priority_v1.json5 1`, and the
  terminal viewer, `cargo run -p tangle-tui -- --backend ascii
...
Nothing in `tangle-model`, `tangle-sim`, `tangle-present`, or the CLI changed, so
...
- `braintree check` — `graph check: passed (90 nodes)`.
