...
`crates/tangle-model/src/source.rs` parses and serializes every Increment 2
...
- `crates/tangle-model/src/mode_template.rs` — adding `TacticKind` variants made
...
- `crates/tangle-model/src/lib.rs` — the new source types join the existing
  `pub use source::{…}` re-export list, which is how every other source type is
  exposed.
- `crates/tangle-model/src/migrate.rs` — the exhaustive `ScenarioSourceV2`
...
- `crates/tangle-model/tests/mode_template_compilation.rs` — three
...
- `schemas/scenario-source.schema.json` — regenerated by
  ...
  untouched).
- `crates/tangle-model/src/schema.rs` and
  `crates/tangle-model/examples/generate-schema.rs` needed no edit: the drift
  test and the generator already cover the new types.

**Tests**: new `crates/tangle-model/tests/increment2_source.rs` (4 tests) covers
...
- `cargo run -p tangle-model --example generate-schema` then a second run —
...
- `cargo test -p tangle-model` — 143 passed, 0 failed (75 lib, then
...
- `braintree check` — `graph check: passed (154 nodes)` while this node was still
  in `proposed/`.
- Post-move `braintree check --allow-pending-advance TAS-082` —
  `graph check: passed (154 nodes)`. Plain `braintree check` now reports only
