...
The accumulation landed in `apps/tangle-cli/src/run_metrics.rs`. The recorder now
...
same node must re-export these new types from `apps/tangle-cli/src/lib.rs` with
it.

Evidence: `cargo test -p tangle-cli --test run_metrics` (9 passed, including
...
`a_centered_facility_reports_no_clearance_value`), `cargo test -p tangle-cli`,
