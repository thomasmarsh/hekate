...
summary: tangle-model parses, validates, and compiles the Increment 0 version-2 subset; SUPPORTED_SCHEMA_VERSION is 2.
---
...
Version 2 is now readable, validatable, and compilable in `tangle-model`.
...
- **Version-2 source shapes** (`crates/tangle-model/src/source.rs`):
...
The focused test `crates/tangle-model/tests/version2_source.rs` shows a
...
Evidence: `cargo test -p tangle-model` (68 tests) passes including both
checked-in-schema drift tests; `cargo test --workspace` passes; `cargo build
--workspace` succeeds; `cargo test -p tangle-cli --test scenarios` passes;
`cargo clippy -p tangle-model --all-targets` is clean;
`scripts/check-dependency-direction.sh` reports `dependency direction OK`;
`braintree check` passes with this node still in `proposed/`.
