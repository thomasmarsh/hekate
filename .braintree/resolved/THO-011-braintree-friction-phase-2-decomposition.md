---
context_rev: 1
updated: 2026-09-12T15:12:57Z
summary: Braintree friction decomposing a requested full plan: the Frontier recipe lists every up-front proposed child, the read loop points at an absent index-map recipe, and THO friction notes are invisible to `feedback scan`.
---
...
`braintree` 0.5.0+gc99a080 and the installed skill on 2026-09-12. This node
...
# Tangle
...
- Indexes [[IDX-001-tangle]].
- Indexes [[IDX-002-braintree-feedback]].
```

There is no `# Focus` list and no recipe section. The recipe text exists only in
SKILL.md under "Common queries". Neither `braintree init` nor `braintree
feedback record` seeds recipes into `nodes/index-map.md`, and `braintree check`
...
`nodes/index-map.md` has none, and optionally have `braintree init` place the
...
`IDX-002-braintree-feedback` per session", and [[IDX-002-braintree-feedback]]
...
feedback from Markdown alone", backed by `braintree feedback record` and
`braintree feedback scan`. This vault follows its repo instruction, so:
...
updated to `braintree feedback record`.
...
- `braintree check nodes` passed after creating nine proposed nodes with
...
- `braintree allocate TAS` returned TAS-017 through TAS-025 with no collisions,
  unlike the recurring stale-ID defect recorded in THO-003 F1 and THO-009 F1.
- `braintree hash`, `claim`, and `release` on [[IDX-001-tangle]] behaved as
