...
- **Shared provenance** (`apps/tangle-cli/src/baseline.rs`): `ScenarioProvenance`
...
- **Run manifest** (`apps/tangle-cli/src/run_dir.rs`): `RUN_MANIFEST_VERSION` is
  2; `RunManifest.scenario` and `RunDirectoryRequest.scenario` carry the shared
  struct unchanged.
- **Version-negotiated loader** (`apps/tangle-cli/src/lib.rs`):
...
`baseline.json`; `cargo test --workspace` passes; `cargo clippy -p tangle-cli
