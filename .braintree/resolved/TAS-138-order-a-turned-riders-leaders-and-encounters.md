...
- `crates/tangle-sim/src/sim.rs (Simulation::nearest_leader)`: own and candidate
...
- `crates/tangle-sim/src/event.rs (Event::order_key)`: the within-tick key reads
...
- `cargo test -p tangle-sim --test wrong_way` 6 passed (the four TAS-117 entry
...
- `cargo test -p tangle-sim` all suites green (no Phase 1 / Increment 1 /
...
Files: `crates/tangle-sim/tests/wrong_way.rs`.
