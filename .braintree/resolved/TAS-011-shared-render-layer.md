...
- New `crates/tangle-present` depends only on `tangle-model`, `tangle-sim`,
...
- `PresentationClock` and its kernel-equivalence tests moved out of
  `tangle-viewer` into `tangle-present`; the scene projection, viewport,
  selection, overlays, and command handling moved with them. `cargo test -p
  tangle-present` runs 22 green tests, and the moved
...
- `tangle-viewer` now builds a `PresentationController` from the loaded
...
- `scripts/check-dependency-direction.sh` now forbids Bevy, terminal crates
  ...
  crates from `tangle-present`, and forbids `tangle-present` from
  `tangle-model` and `tangle-sim`. CI already runs the script, which reports
