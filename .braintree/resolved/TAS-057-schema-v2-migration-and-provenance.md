...
- **Deterministic migration.** [[TAS-063-v1-to-v2-migration-and-cli]] added
  ...
  normalized serializer, and the `tangle-cli migrate` subcommand, with demand and
...
- **Phase 1 regression.** [[TAS-065-phase-1-migration-regression]] added
  `apps/tangle-cli/tests/migration_regression.rs`: every checked-in version-1
...
Evidence: `braintree check` passes (110 nodes); `cargo test --workspace` passes;
...
migration suites in `tangle-model` pass; `scripts/check-dependency-direction.sh`
