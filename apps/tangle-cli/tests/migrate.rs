...
/// The checked-in migration fixtures, shared with `tangle-model`'s tests.
const FIXTURE_DIR: &str = "crates/tangle-model/tests/fixtures/migration";
...
impl Scratch {
    fn new(name: &str) -> Self {
        let dir =
            std::env::temp_dir().join(format!("tangle-cli-migrate-{name}-{}", std::process::id()));
