...
use tangle_sim::{Event, RunConfig, Seconds};
...
impl Scratch {
    fn new(name: &str) -> Self {
        let dir = std::env::temp_dir().join(format!(
            "tangle-narrow-determinism-{name}-{}",
            std::process::id()
        ));
...
fn batch_request(
    ...
    BatchRequest {
        ...
        sampling: SamplingPolicy {
            policy_version: tangle_cli::SAMPLING_POLICY_VERSION,
...
fn read_run_manifest(directory: &Path) -> RunManifest {
    let json = std::fs::read_to_string(directory.join(tangle_cli::MANIFEST_FILE))
...
fn the_narrow_determinism_executes_the_fixtures() {
    for (id, path) in FIXTURES {
        let (scenario, _) = load_scenario_provenance(&repo_path(path)).expect("the fixture loads");
        let mut sim = tangle_sim::Simulation::new(scenario, config(SEED, 0.05))
