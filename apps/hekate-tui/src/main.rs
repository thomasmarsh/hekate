//! `hekate-tui`: a character-cell terminal viewer for Hekate.
//!
...
use hekate_present::{Overlay, RestartMode, Speed, ViewCommand, load_scenario};
use hekate_tui::capability::{
    self, BackendKind, BackendRequest, EnvironmentHints, TerminalResponder,
};
use hekate_tui::terminal::{RealTerminal, TerminalModes};
use hekate_tui::{
...
/// Command-line help.
const USAGE: &str = "\
usage: hekate-tui [--backend ascii|kitty|auto] [scenario] [seed]
...
fn handle_key(session: &mut ViewerSession, key: KeyEvent) -> bool {
    match key.code {
        ...
        KeyCode::Char('c') => session.apply(ViewCommand::ToggleOverlay(Overlay::Corridor)),
        KeyCode::Char('t') => session.apply(ViewCommand::ToggleOverlay(Overlay::TargetOffset)),
