...
//!
//! Run with `cargo run -p hekate-tui --release --example kitty-bench`.
//!
//! By default the backend writes to a discarding sink, which measures raster
//! and base64 encode cost only. Set `HEKATE_BENCH_PTY=1` and run the binary
//! under a pty (for example `script -q /dev/null ...`) to include the terminal
//! write cost. `HEKATE_BENCH_FRAMES` overrides the frame count.
//!
...
fn main() -> Result<(), BoxError> {
    ...
    let frames = frames();
    if std::env::var_os("HEKATE_BENCH_PTY").is_some() {
        println!(
            "{}",
...
fn frames() -> u32 {
    std::env::var("HEKATE_BENCH_FRAMES")
        .ok()
