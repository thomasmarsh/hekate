...
use hekate_sim::{InitError, RunConfig, Seconds};
...
mod tests {
    ...
    fn an_absent_run_directory_is_reported_as_missing() {
        let directory =
            std::env::temp_dir().join(format!("hekate-replay-absent-{}", std::process::id()));
    ...
    fn a_directory_without_a_manifest_is_reported_as_incomplete() {
        let directory =
            std::env::temp_dir().join(format!("hekate-replay-incomplete-{}", std::process::id()));
        let _ = fs::remove_dir_all(&directory);
