...
//!
//! The Parquet dependency lives here, in the CLI/output layer: `hekate-model`
//! and `hekate-sim` never depend on it.
...
mod tests {
    ...
    fn a_recorded_sample_carries_the_frames_the_kernel_reports() {
        ...
        let source = hekate_model::parse_scenario_source(WALKING).expect("scenario parses");
        let scenario = hekate_model::CompiledScenario::compile(source).expect("scenario compiles");
        let mut sim = Simulation::new(scenario, hekate_sim::RunConfig::new(0)).expect("builds");
    ...
    fn the_artifact_round_trips_optional_route_state() {
        let present = TrajectorySample {
            ...
            body_kind: hekate_model::BodyKind::Capsule,
            x_m: 1.0,
            y_m: 2.0,
            heading_rad: 0.5,
            speed_mps: 3.0,
    ...
    fn an_opposing_run_records_its_rule_state_into_the_artifact() {
        ...
        let mut sim = Simulation::new(compile(), hekate_sim::RunConfig::new(0))
