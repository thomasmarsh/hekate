...
//! `no_inc2_golden_records_two_agents_maneuvering_on_one_tick` enforces — so
//! these goldens cannot cover the batch arbitration's tie-break. That ordering is
//! proved by TAS-127's unit and fixed-seed integration suites
//! (`crates/hekate-sim/tests/claims.rs`), which is the ordering evidence this
//! node reports; no scenario was authored to manufacture a second simultaneous
//! claim.
//!
...
/// required presets, and the golden's own header names the preset it was
/// recorded at.
#[test]
fn every_inc2_fixture_matches_its_golden_bytes_and_hash_at_both_required_presets() {
...
/// or the wrong-way interval lifecycle that fixture exists to prove.
///
/// The transitions are read out of the recorded bytes rather than out of the
/// live run, so the coverage claim is about the checked-in artifact a reviewer
/// can read: the attempt, commit, and completion edges of the lifecycle; the
...
/// traversal.
#[test]
fn every_inc2_golden_covers_its_maneuver_transitions_and_the_wrong_way_lifecycle() {
    for fixture in FIXTURES {
        for (preset, _) in PRESETS {
            let bytes = golden_text(&fixture, preset, "jsonl").into_bytes();
            let maneuvers: Vec<(String, String, String, String)> =
                events_of_kind(&bytes, "maneuver")
                    .iter()
                    .map(maneuver_record)
                    .collect();
...
/// No golden records two agents' maneuver transitions on one tick, so these
/// goldens are not — and must not be read as — simultaneous-claim ordering
/// evidence.
///
...
/// (TAS-127).
#[test]
fn no_inc2_golden_records_two_agents_maneuvering_on_one_tick() {
    for fixture in FIXTURES {
        for (preset, _) in PRESETS {
            let bytes = golden_text(&fixture, preset, "jsonl").into_bytes();
