...
//!
//! ```sh
//! cargo run -p hekate-cli -- run scenarios/experiments/four_leg_pedestrian_ew_priority_v1.json5 \
//!   --seed 1 --ticks 1200 \
...
//! cargo run -p hekate-cli -- run scenarios/experiments/four_leg_pedestrian_ns_priority_v1.json5 \
//!   --seed 1 --ticks 1200 \
...
impl Scratch {
    fn new(name: &str) -> Self {
        let dir = std::env::temp_dir().join(format!(
            "hekate-cli-increment6-trace-{name}-{}",
            std::process::id()
        ));
