...
impl Scratch {
    fn new(name: &str) -> Self {
        let dir =
            std::env::temp_dir().join(format!("hekate-cli-run-dir-{name}-{}", std::process::id()));
...
fn walking() -> (hekate_model::CompiledScenario, ScenarioProvenance) {
...
fn request<'a>(
    ...
    trace: &'a hekate_cli::Trace,
    summary: &'a hekate_sim::RunSummary,
...
fn write_golden_run(directory: &Path) {
    let (provenance, trace, summary, trajectories, metrics) = golden_run();
    write_run_directory(
        directory,
        request(&provenance, &trace, &summary, &trajectories, &metrics),
    )
...
fn manifest_records_the_provenance_a_rerun_needs() {
    ...
    assert_eq!(
        ...
        hekate_model::MIGRATION_VERSION
    );
    ...
    assert_eq!(manifest.model_version, hekate_model::MODEL_VERSION);
