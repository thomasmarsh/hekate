...
(`braintree hash TAS-039`, bare-ID form) with `--lease-seconds 14400`. Write set:
...
- `apps/tangle-cli/src/seed_bank.rs` (new) owns the artifact. `SeedBank` holds
  ...
  and no randomness.
- `apps/tangle-cli/src/main.rs` adds the `seed-bank` command (`--seeds` xor
...
- `apps/tangle-cli/src/batch.rs` adds `BatchRequest.seed_bank` and
...
- `apps/tangle-cli/tests/seed_bank.rs` (new) is the contract suite;
...
- `apps/tangle-cli/Cargo.toml` and `Cargo.lock` are unchanged.

## Kernel seed-to-stream derivation (read only)

`crates/tangle-sim/src/rng.rs` derives every named stream from the root seed:
...
`apps/tangle-cli/tests/seed_bank.rs` (9 tests):
