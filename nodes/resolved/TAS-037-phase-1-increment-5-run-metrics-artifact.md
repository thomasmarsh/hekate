...
- `apps/tangle-cli/src/run_metrics.rs` (new) owns the artifact.
...
the still-live simulation and disaggregates them. `RunMetricsArtifact` is the
  ...
  never a false `0`.
- `apps/tangle-cli/src/run_dir.rs` adds `RunDirectoryRequest.metrics` and
  ...
  unchanged.
- `apps/tangle-cli/src/trace.rs` adds `canonical_run_captured`, the one run
...
- `apps/tangle-cli/src/main.rs` uses the capturing loop when `--run-dir` is
  given; `apps/tangle-cli/src/batch.rs` does the same per seed and adds
  ...
  surface.
- `crates/tangle-sim/src/metrics.rs` (widened, read-only and additive) adds
...
- `apps/tangle-cli/tests/run_metrics.rs` (new) is the contract suite.
...
from `braintree hash TAS-037`, claimed by `worker` for 14400 s. Write set:
`apps/tangle-cli/src/**`, `apps/tangle-cli/tests/**`, and this node's own
frontier transition, plus the supervisor-sanctioned widening
`crates/tangle-sim/src/metrics.rs`. Every changed, created, and moved path
...
changed, and `EVENT_VERSION` stays 2 (`crates/tangle-sim/src/event.rs`). The
...
`apps/tangle-cli/tests/run_metrics.rs` (4 tests), the new kernel test
