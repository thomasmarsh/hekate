...
Implemented in `crates/tangle-sim`:
...
Evidence: `cargo test --workspace --all-features` passes 18 `tangle-sim` unit
...
`dependency direction OK`, confirming no Bevy dependency reaches `tangle-sim`.
