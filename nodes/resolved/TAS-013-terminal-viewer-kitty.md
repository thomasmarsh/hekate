...
All Done-when criteria are met. `tangle-tui` has an opt-in Kitty graphics
...
`cargo run -p tangle-tui --release --example kitty-bench` (240 frames; includes
...
`cargo test -p tangle-tui` is 63 library tests plus 3 argument tests plus
