...
Implemented in `apps/tangle-cli`:
...
`apps/tangle-cli/tests/golden_trace.rs` regenerates the trace from the
...
`tangle-cli` unit tests and 2 golden-trace integration tests;
...
dependency; no Bevy type enters `tangle-model`, `tangle-sim`, or `tangle-cli`.
