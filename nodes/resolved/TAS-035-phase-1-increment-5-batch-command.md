...
- `apps/tangle-cli/src/batch.rs` (new) owns the batch. `run_batch` (`:204`)
...
- `apps/tangle-cli/src/main.rs` adds `batch` (`:291`) with `BatchArgs` (`:266`):
...
- `apps/tangle-cli/src/run_dir.rs:482` makes the existing `fidelity` helper
  `pub(crate)` so the batch spec labels a fixed step exactly as a run manifest
  does, rather than duplicating the preset table.
- `apps/tangle-cli/tests/batch.rs` (new) is the contract suite.

## Evidence

`cargo test --workspace --all-features` (479 passed, 0 failed) on this tree,
including the new `apps/tangle-cli/tests/batch.rs` (6 tests) and the 3 new unit
tests in `batch.rs`:
...
from the standard library, so `apps/tangle-cli/Cargo.toml` and `Cargo.lock` are
unchanged and no concurrency helper was added.

## Preservation

No file under `tests/golden/`, `baselines/`, `scenarios/`, `schemas/`, or the
kernel crates changed; `EVENT_VERSION` stays 2 (`crates/tangle-sim/src/event.rs`).
...
5. **The canonical trace, its hash golden, the Phase 1 baseline, and all goldens
   ...
   only `apps/tangle-cli/src/{batch.rs,lib.rs,main.rs,run_dir.rs}` and
   `apps/tangle-cli/tests/batch.rs` plus the node move; no file under
   `tests/golden/`, `baselines/`, `scenarios/`, `schemas/`, or the kernel crates
   changed, and `EVENT_VERSION` stays 2
   (`crates/tangle-sim/src/event.rs:66`). The golden and baseline guards
   (`tests/golden_trace.rs`, `tests/baseline.rs`, `tests/run_directory.rs`,
   `tests/trajectories.rs`) still pass, and the batch reuses
   `canonical_run_sampled` (`apps/tangle-cli/src/trace.rs:150`) unchanged.
6. **The five gates pass on the final tree** (rerun on `2287b68`, 2026-09-13:
   ...
   `braintree check nodes` printed `graph check: passed (61 nodes)`).
